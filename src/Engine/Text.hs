...
do_typesetting height calculate_typesetting seq_seq_row=do_typesetting_a (-height) (`calculate_typesetting` seq_seq_row) 1 seq_seq_row
...
do_typesetting_b y calculate_typesetting row_number seq_row=case seq_row of
    ...
    (row DS.:<| other_row)->let (x,new_y)=calculate_typesetting row_number in let new_new_y=y+new_y in let (final_y,final_row_number,final_seq_row)=do_typesetting_b new_new_y calculate_typesetting (row_number+1) other_row in case row of
        ...
        Row {seq_character,width,min_down,max_up,min_descent,max_ascent}->(final_y,final_row_number,Row {seq_character=seq_character,x=x-width/2,y=new_new_y,width=width,min_down=min_down,max_up=max_up,min_descent=min_descent,max_ascent=max_ascent} DS.:<| final_seq_row)
