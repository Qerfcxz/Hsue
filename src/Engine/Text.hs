...
for_text_b index font font_map paragraph min_down max_up min_descent max_ascent x width calculate_width row_core this_paragraph article=case paragraph of
    DS.Empty->if min_down==1/0||max_up==(negate 1/0)||min_descent==1/0||max_ascent==(negate 1/0) then (article DS.|> DS.empty,index+1) else (article DS.|> (this_paragraph DS.|> Row {row_core=row_core,index=index,x=0,y=0,width=x,min_down=min_down,max_up=max_up,min_descent=min_descent,max_ascent=max_ascent}),index+1)
