...
do_typesetting_a y calculate_typesetting row_number article=case article of
    DS.Empty->DS.empty
    (paragraph DS.:<| other_paragraph)->let (next_y,new_row_number,new_paragraph)=do_typesetting_b y calculate_typesetting row_number paragraph in new_paragraph DS.<| do_typesetting_a next_y calculate_typesetting new_row_number other_paragraph
...
do_typesetting_b y calculate_typesetting row_number paragraph=case paragraph of
    ...
    (row DS.:<| other_row)->let (x,new_y)=calculate_typesetting row_number in let new_new_y=y+new_y in let (final_y,final_row_number,final_paragraph)=do_typesetting_b new_new_y calculate_typesetting (row_number+1) other_row in case row of
        Blank->(final_y,final_row_number,Blank DS.:<| final_paragraph)
        Row {row_core,width,min_down,max_up,min_descent,max_ascent}->(final_y,final_row_number,Row {row_core=row_core,x=x-width/2,y=new_new_y,width=width,min_down=min_down,max_up=max_up,min_descent=min_descent,max_ascent=max_ascent} DS.:<| final_paragraph)
