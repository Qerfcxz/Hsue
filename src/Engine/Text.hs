...
for_text::ET.Has_call_stack=>Int->DIM.IntMap Font->DHMS.HashMap DT.Text Int->DS.Seq (DS.Seq Sentence)->(DS.Seq Row->DS.Seq (DS.Seq Row)->Int->FCT.CFloat)->(DS.Seq (DS.Seq Row),Int)
for_text max_search_index font font_map article calculate_width=for_text_a 0 max_search_index font font_map article calculate_width DS.empty

for_text_a::ET.Has_call_stack=>Int->Int->DIM.IntMap Font->DHMS.HashMap DT.Text Int->DS.Seq (DS.Seq Sentence)->(DS.Seq Row->DS.Seq (DS.Seq Row)->Int->FCT.CFloat)->DS.Seq (DS.Seq Row)->(DS.Seq (DS.Seq Row),Int)
for_text_a index max_search_index font font_map article calculate_width this_article=case article of
    DS.Empty->(this_article,index)
    paragraph DS.:<| other_paragraph->let (new_article,new_index)=for_text_b index max_search_index font font_map paragraph (1/0) (negate 1/0) (1/0) (negate 1/0) 0 (calculate_width DS.empty this_article index) calculate_width DS.empty DS.empty this_article in for_text_a new_index max_search_index font font_map other_paragraph calculate_width new_article

for_text_b::ET.Has_call_stack=>Int->Int->DIM.IntMap Font->DHMS.HashMap DT.Text Int->DS.Seq Sentence->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->(DS.Seq Row->DS.Seq (DS.Seq Row)->Int->FCT.CFloat)->DS.Seq Character->DS.Seq Row->DS.Seq (DS.Seq Row)->(DS.Seq (DS.Seq Row),Int)
for_text_b index max_search_index font font_map paragraph min_down max_up min_descent max_ascent x width calculate_width row_core this_paragraph article=case paragraph of
...
for_text_c::ET.Has_call_stack=>Int->Int->Int->DIM.IntMap Glyph->DS.Seq Phrase->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->(DS.Seq Row->Int->FCT.CFloat)->DS.Seq Character->DS.Seq Row->(DS.Seq Row,DS.Seq Character,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,Int)
for_text_c index max_search_index font_id glyph sentence_core min_down max_up min_descent max_ascent descent ascent x width calculate_width row_core paragraph=case sentence_core of
    DS.Empty->(paragraph,row_core,width,x,max_ascent,min_descent,max_up,min_down,index)
    phrase DS.:<| other_phrase->case phrase of
        Phrase {phrase_core,font_size,color}->let (new_paragraph,new_row_core,new_width,new_x,new_max_ascent,new_min_descent,new_max_up,new_min_down,new_index)=let new_descent=descent*font_size in let new_ascent=ascent*font_size in for_text_d index max_search_index font_id glyph phrase_core min_down max_up (min new_descent min_descent) (max new_ascent max_ascent) new_descent new_ascent font_size x width color calculate_width row_core paragraph in for_text_c new_index max_search_index font_id glyph other_phrase new_min_down new_max_up new_min_descent new_max_ascent descent ascent new_x new_width calculate_width new_row_core new_paragraph

for_text_d::ET.Has_call_stack=>Int->Int->Int->DIM.IntMap Glyph->DT.Text->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->Color->(DS.Seq Row->Int->FCT.CFloat)->DS.Seq Character->DS.Seq Row->(DS.Seq Row,DS.Seq Character,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,Int)
for_text_d index max_search_index font_id glyph text min_down max_up min_descent max_ascent descent ascent font_size x width color calculate_width row_core paragraph=case text of
...
from_charset_a path charset engine=let unicode=DHS.foldl' (\this_unicode char->DIS.insert (DC.ord char) this_unicode) DIS.empty charset in case DHMS.lookup path engine.font_map of
    Nothing->if DIS.null unicode then return (engine {font_map=hash_map_insert_strict path engine.font_id engine.font_map,font=DIM.insert engine.font_id (Font {glyph=DIM.empty,descent=0,ascent=0}) engine.font,font_id=engine.font_id+1}) else from_charset_b False DIM.empty engine.font_id path unicode (engine {font_map=hash_map_insert_strict path engine.font_id engine.font_map,font_id=engine.font_id+1})
