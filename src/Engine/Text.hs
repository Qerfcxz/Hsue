...
for_text::ET.Has_call_stack=>FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->Int->DIM.IntMap Font->DHMS.HashMap DT.Text Int->DS.Seq (DS.Seq Sentence)->(DS.Seq Row->DS.Seq (DS.Seq Row)->Int->FCT.CFloat)->(DS.Seq (DS.Seq Row),Int)
for_text u v failure_advance failure_left failure_down failure_right failure_up max_search_index font font_map article calculate_width=for_text_a u v failure_advance failure_left failure_down failure_right failure_up 0 max_search_index font font_map article calculate_width DS.empty

for_text_a::ET.Has_call_stack=>FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->Int->Int->DIM.IntMap Font->DHMS.HashMap DT.Text Int->DS.Seq (DS.Seq Sentence)->(DS.Seq Row->DS.Seq (DS.Seq Row)->Int->FCT.CFloat)->DS.Seq (DS.Seq Row)->(DS.Seq (DS.Seq Row),Int)
for_text_a u v failure_advance failure_left failure_down failure_right failure_up index max_search_index font font_map article calculate_width this_article=case article of
    DS.Empty->(this_article,index)
    paragraph DS.:<| other_paragraph->let (new_article,new_index)=for_text_b index max_search_index font font_map paragraph (1/0) (negate 1/0) (1/0) (negate 1/0) 0 (calculate_width DS.empty this_article index) u v failure_advance failure_left failure_down failure_right failure_up calculate_width DS.empty DS.empty this_article in for_text_a u v failure_advance failure_left failure_down failure_right failure_up new_index max_search_index font font_map other_paragraph calculate_width new_article

for_text_b::ET.Has_call_stack=>Int->Int->DIM.IntMap Font->DHMS.HashMap DT.Text Int->DS.Seq Sentence->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->(DS.Seq Row->DS.Seq (DS.Seq Row)->Int->FCT.CFloat)->DS.Seq Character->DS.Seq Row->DS.Seq (DS.Seq Row)->(DS.Seq (DS.Seq Row),Int)
for_text_b index max_search_index font font_map paragraph min_down max_up min_descent max_ascent x width u v failure_advance failure_left failure_down failure_right failure_up calculate_width row_core this_paragraph article=case paragraph of
    DS.Empty->(article DS.|> (if DS.null row_core then this_paragraph else this_paragraph DS.|> Row {row_core=row_core,index=index,x=0,y=0,width=x,min_down=min_down,max_up=max_up,min_descent=min_descent,max_ascent=max_ascent}),index+1)
    sentence DS.:<| other_sentence->case sentence of
        Sentence {sentence_core,path}->let font_id=hash_map_lookup path font_map in let single_font=int_map_lookup font_id font in let (new_paragraph,new_row_core,new_width,new_x,new_max_ascent,new_min_descent,new_max_up,new_min_down,new_index)=for_text_c index max_search_index font_id single_font.glyph sentence_core min_down max_up min_descent max_ascent single_font.descent single_font.ascent x width u v failure_advance failure_left failure_down failure_right failure_up (`calculate_width` article) row_core this_paragraph in for_text_b new_index max_search_index font font_map other_sentence new_min_down new_max_up new_min_descent new_max_ascent new_x new_width u v failure_advance failure_left failure_down failure_right failure_up calculate_width new_row_core new_paragraph article

for_text_c::ET.Has_call_stack=>Int->Int->Int->DIM.IntMap Glyph->DS.Seq Phrase->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->(DS.Seq Row->Int->FCT.CFloat)->DS.Seq Character->DS.Seq Row->(DS.Seq Row,DS.Seq Character,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,Int)
for_text_c index max_search_index font_id glyph sentence_core min_down max_up min_descent max_ascent descent ascent x width u v failure_advance failure_left failure_down failure_right failure_up calculate_width row_core paragraph=case sentence_core of
    DS.Empty->(paragraph,row_core,width,x,max_ascent,min_descent,max_up,min_down,index)
    phrase DS.:<| other_phrase->case phrase of
        Phrase {phrase_core,font_size,color}->let (new_paragraph,new_row_core,new_width,new_x,new_max_ascent,new_min_descent,new_max_up,new_min_down,new_index)=let new_descent=descent*font_size in let new_ascent=ascent*font_size in for_text_d index max_search_index font_id glyph phrase_core min_down max_up (min new_descent min_descent) (max new_ascent max_ascent) new_descent new_ascent font_size x width u v failure_advance failure_left failure_down failure_right failure_up color calculate_width row_core paragraph in for_text_c new_index max_search_index font_id glyph other_phrase new_min_down new_max_up new_min_descent new_max_ascent descent ascent new_x new_width u v failure_advance failure_left failure_down failure_right failure_up calculate_width new_row_core new_paragraph

for_text_d::ET.Has_call_stack=>Int->Int->Int->DIM.IntMap Glyph->DT.Text->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->Color->(DS.Seq Row->Int->FCT.CFloat)->DS.Seq Character->DS.Seq Row->(DS.Seq Row,DS.Seq Character,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,FCT.CFloat,Int)
for_text_d index max_search_index font_id glyph text min_down max_up min_descent max_ascent descent ascent font_size x width u v failure_advance failure_left failure_down failure_right failure_up color calculate_width row_core paragraph=case text of
    DT.Empty->(paragraph,row_core,width,x,max_ascent,min_descent,max_up,min_down,index)
    char DT.:< other_char->let unicode=DC.ord char in case DIM.lookup unicode glyph of
        Nothing->let new_advance=failure_advance*font_size in let new_left=failure_left*font_size in let new_down=failure_down*font_size in let new_right=failure_right*font_size in let new_up=failure_up*font_size in if width<x+new_right then let new_paragraph=if DS.null row_core then paragraph else paragraph DS.|> Row {row_core=row_core,index=index,x=0,y=0,width=x,min_down=min_down,max_up=max_up,min_descent=min_descent,max_ascent=max_ascent} in let max_index=index+1+max_search_index in let (new_width,new_index)=for_text_e (index+1) max_index new_right calculate_width new_paragraph in for_text_d new_index max_search_index font_id glyph other_char new_down new_up descent ascent descent ascent font_size new_advance new_width u v failure_advance failure_left failure_down failure_right failure_up color calculate_width (DS.singleton (Character {unicode=unicode,font_id=font_id,font_size=font_size,left=new_left,down=new_down,right=new_right,up=new_up,min_u=u,min_v=v,max_u=u,max_v=v,color=color})) new_paragraph else for_text_d index max_search_index font_id glyph other_char (min new_down min_down) (max new_up max_up) min_descent max_ascent descent ascent font_size (x+new_advance) width u v failure_advance failure_left failure_down failure_right failure_up color calculate_width (row_core DS.|> Character {unicode=unicode,font_id=font_id,font_size=font_size,left=x+new_left,down=new_down,right=x+new_right,up=new_up,min_u=u,min_v=v,max_u=u,max_v=v,color=color}) paragraph
        Just (Glyph {advance,left,down,right,up,min_u,min_v,max_u,max_v})->let new_advance=advance*font_size in let new_left=left*font_size in let new_down=down*font_size in let new_right=right*font_size in let new_up=up*font_size in if width<x+new_right then let new_paragraph=if DS.null row_core then paragraph else paragraph DS.|> Row {row_core=row_core,index=index,x=0,y=0,width=x,min_down=min_down,max_up=max_up,min_descent=min_descent,max_ascent=max_ascent} in let max_index=index+1+max_search_index in let (new_width,new_index)=for_text_e (index+1) max_index new_right calculate_width new_paragraph in for_text_d new_index max_search_index font_id glyph other_char new_down new_up descent ascent descent ascent font_size new_advance new_width u v failure_advance failure_left failure_down failure_right failure_up color calculate_width (DS.singleton (Character {unicode=unicode,font_id=font_id,font_size=font_size,left=new_left,down=new_down,right=new_right,up=new_up,min_u=min_u,min_v=min_v,max_u=max_u,max_v=max_v,color=color})) new_paragraph else for_text_d index max_search_index font_id glyph other_char (min new_down min_down) (max new_up max_up) min_descent max_ascent descent ascent font_size (x+new_advance) width u v failure_advance failure_left failure_down failure_right failure_up color calculate_width (row_core DS.|> Character {unicode=unicode,font_id=font_id,font_size=font_size,left=x+new_left,down=new_down,right=x+new_right,up=new_up,min_u=min_u,min_v=min_v,max_u=max_u,max_v=max_v,color=color}) paragraph

for_text_e::ET.Has_call_stack=>Int->Int->FCT.CFloat->(DS.Seq Row->Int->FCT.CFloat)->DS.Seq Row->(FCT.CFloat,Int)
for_text_e index max_index right calculate_width paragraph=let width=calculate_width paragraph index in if right<=width||index==max_index then (width,index) else for_text_e (index+1) max_index right calculate_width paragraph

to_charset::ET.Has_call_stack=>DS.Seq (DS.Seq Sentence)->DHMS.HashMap DT.Text (DHS.HashSet Char)
to_charset=DF.foldl' (DF.foldl' (flip to_charset_a)) DHMS.empty

to_charset_a::ET.Has_call_stack=>Sentence->DHMS.HashMap DT.Text (DHS.HashSet Char)->DHMS.HashMap DT.Text (DHS.HashSet Char)
...
update_font path maybe_charset engine=case DHMS.lookup path engine.font_map of
    Nothing->update_font_a DIM.empty engine.font_id path maybe_charset (engine {font_map=hash_map_insert_strict path engine.font_id engine.font_map,font_id=engine.font_id+1})
    Just font_id->let single_font=int_map_lookup font_id engine.font in update_font_a single_font.glyph font_id path maybe_charset engine

update_font_a::ET.Has_call_stack=>DIM.IntMap Glyph->Int->DT.Text->Maybe (DHS.HashSet Char)->Engine a->IO (Engine a)
update_font_a glyph font_id path maybe_charset engine=case maybe_charset of
    ...
    Just charset->let unicode=DHS.foldl' (\this_unicode char->DIS.insert (DC.ord char) this_unicode) DIS.empty charset in case DIM.lookup font_id engine.font of
...
from_charset_a path charset engine=let unicode=DHS.foldl' (\this_unicode char->DIS.insert (DC.ord char) this_unicode) DIS.empty charset in case DHMS.lookup path engine.font_map of
    ...
    Just font_id->let single_font=int_map_lookup font_id engine.font in let new_unicode=DIS.difference unicode (DIM.keysSet single_font.glyph) in if DIS.null new_unicode then return engine else from_charset_b False single_font.glyph font_id path new_unicode engine
...
scroll_text strict_match scroll visual=case visual of
    Text {arrange,half_width,half_height,failure_advance,failure_left,failure_down,failure_right,failure_up,current_y,min_y,max_y,anchor,article,charset,locked}->Text {arrange=arrange,half_width=half_width,half_height=half_height,failure_advance=failure_advance,failure_left=failure_left,failure_down=failure_down,failure_right=failure_right,failure_up=failure_up,current_y=max min_y (min (max min_y max_y) (current_y+scroll)),min_y=min_y,max_y=max_y,anchor=anchor,article=article,charset=charset,locked=locked}
...
scroll_top_text strict_match visual=case visual of
    Text {arrange,half_width,half_height,failure_advance,failure_left,failure_down,failure_right,failure_up,min_y,max_y,anchor,article,charset,locked}->Text {arrange=arrange,half_width=half_width,half_height=half_height,failure_advance=failure_advance,failure_left=failure_left,failure_down=failure_down,failure_right=failure_right,failure_up=failure_up,current_y=min_y,min_y=min_y,max_y=max_y,anchor=anchor,article=article,charset=charset,locked=locked}
    _->if strict_match then EF.empty_error else visual

scroll_bottom_text::ET.Has_call_stack=>Bool->Visual a->Visual a
scroll_bottom_text strict_match visual=case visual of
    Text {arrange,half_width,half_height,failure_advance,failure_left,failure_down,failure_right,failure_up,min_y,max_y,anchor,article,charset,locked}->Text {arrange=arrange,half_width=half_width,half_height=half_height,failure_advance=failure_advance,failure_left=failure_left,failure_down=failure_down,failure_right=failure_right,failure_up=failure_up,current_y=max min_y max_y,min_y=min_y,max_y=max_y,anchor=anchor,article=article,charset=charset,locked=locked}
    _->if strict_match then EF.empty_error else visual
