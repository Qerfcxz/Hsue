...
for_text_b font seq_sentence min_down max_up min_descent max_ascent x width calculate_width row_number seq_character seq_row seq_seq_row=case seq_sentence of
    ...
    (sentence DS.:<| other_sentence)->case sentence of
        Sentence {seq_phrase,font_id}->let new_font=intmap_lookup font_id font in let (new_min_down,new_max_up,new_min_descent,new_max_ascent,new_x,new_width,new_row_number,new_seq_character,new_seq_row)=for_text_c font_id new_font.glyph seq_phrase min_down max_up min_descent max_ascent new_font.descent new_font.ascent x width calculate_width row_number seq_character seq_row seq_seq_row in for_text_b font other_sentence new_min_down new_max_up new_min_descent new_max_ascent new_x new_width calculate_width new_row_number new_seq_character new_seq_row seq_seq_row

for_text_c::Int->DIM.IntMap Glyph->DS.Seq Phrase->Extended FCT.CFloat->Extended FCT.CFloat->Extended FCT.CFloat->Extended FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->(Int->DS.Seq Row->DS.Seq (DS.Seq Row)->FCT.CFloat)->Int->DS.Seq Character->DS.Seq Row->DS.Seq (DS.Seq Row)->(Extended FCT.CFloat,Extended FCT.CFloat,Extended FCT.CFloat,Extended FCT.CFloat,FCT.CFloat,FCT.CFloat,Int,DS.Seq Character,DS.Seq Row)
for_text_c font_id glyph seq_phrase min_down max_up min_descent max_ascent descent ascent x width calculate_width row_number seq_character seq_row seq_seq_row=case seq_phrase of
    ...
    (phrase DS.:<| other_phrase)->case phrase of
        Phrase {text,size,red,green,blue,alpha}->let (new_min_down,new_max_up,new_min_descent,new_max_ascent,new_x,new_width,new_row_number,new_seq_character,new_seq_row)=let new_descent=descent*size in let new_ascent=ascent*size in for_text_d font_id glyph text min_down max_up (min (to_extended new_descent) min_descent) (max (to_extended new_ascent) max_ascent) new_descent new_ascent size red green blue alpha x width calculate_width row_number seq_character seq_row seq_seq_row in for_text_c font_id glyph other_phrase new_min_down new_max_up new_min_descent new_max_ascent descent ascent new_x new_width calculate_width new_row_number new_seq_character new_seq_row seq_seq_row

for_text_d::Int->DIM.IntMap Glyph->DT.Text->Extended FCT.CFloat->Extended FCT.CFloat->Extended FCT.CFloat->Extended FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->(Int->DS.Seq Row->DS.Seq (DS.Seq Row)->FCT.CFloat)->Int->DS.Seq Character->DS.Seq Row->DS.Seq (DS.Seq Row)->(Extended FCT.CFloat,Extended FCT.CFloat,Extended FCT.CFloat,Extended FCT.CFloat,FCT.CFloat,FCT.CFloat,Int,DS.Seq Character,DS.Seq Row)
for_text_d font_id glyph text min_down max_up min_descent max_ascent descent ascent size red green blue alpha x width calculate_width row_number seq_character seq_row seq_seq_row=case text of
    ...
    (char DT.:< other_char)->let unicode=DC.ord char in case intmap_lookup unicode glyph of
        Glyph {advance,left,down,right,up,min_u,min_v,max_u,max_v}->let new_advance=advance*size in let new_down=down*size in let new_right=right*size in let new_up=up*size in let new_new_down=to_extended new_down in let new_new_up=to_extended new_up in if width<x+new_right then let (new_width,new_row_number,new_seq_row)=for_text_e new_right calculate_width (row_number+1) (seq_row DS.|> Row {seq_character=seq_character,x=0,y=0,width=x,min_down=from_extended min_down,max_up=from_extended max_up,min_descent=from_extended min_descent,max_ascent=from_extended max_ascent}) seq_seq_row in for_text_d font_id glyph other_char new_new_down new_new_up (to_extended descent) (to_extended ascent) descent ascent size red green blue alpha new_advance new_width calculate_width new_row_number (DS.singleton (Character {unicode=unicode,size=size,font_id=font_id,left=left*size,down=new_down,right=new_right,up=new_up,min_u=min_u,min_v=min_v,max_u=max_u,max_v=max_v,red=red,green=green,blue=blue,alpha=alpha})) new_seq_row seq_seq_row else for_text_d font_id glyph other_char (min new_new_down min_down) (max new_new_up max_up) min_descent max_ascent descent ascent size red green blue alpha (x+new_advance) width calculate_width row_number (seq_character DS.|> Character {unicode=unicode,size=size,font_id=font_id,left=x+left*size,down=new_down,right=x+new_right,up=new_up,min_u=min_u,min_v=min_v,max_u=max_u,max_v=max_v,red=red,green=green,blue=blue,alpha=alpha}) seq_row seq_seq_row

for_text_e::FCT.CFloat->(Int->DS.Seq Row->DS.Seq (DS.Seq Row)->FCT.CFloat)->Int->DS.Seq Row->DS.Seq (DS.Seq Row)->(FCT.CFloat,Int,DS.Seq Row)
for_text_e right calculate_width row_number seq_row seq_seq_row=let width=calculate_width row_number seq_row seq_seq_row in if right<width then (width,row_number,seq_row) else for_text_e right calculate_width (row_number+1) (seq_row DS.|> Blank) seq_seq_row