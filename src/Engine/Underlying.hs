...
lock_visual visual=case visual of
    ...
    Text {arrange,half_width,half_height,current_y,min_y,max_y,anchor,hole_index,hole,article,charset}->Text {arrange=arrange,half_width=half_width,half_height=half_height,current_y=current_y,min_y=min_y,max_y=max_y,anchor=anchor,hole_index=hole_index,hole=hole,article=article,charset=charset,locked=True}
