...
int_map_applicative_update update first_int_map second_int_map=if DIS.isSubsetOf (DIM.keysSet first_int_map) (DIM.keysSet second_int_map) then DIM.traverseWithKey (\key value->int_map_applicative_update_a key update first_int_map value) second_int_map else EE.empty_error
...
int_set_insert key int_set=if DIS.member key int_set then EE.empty_error else DIS.insert key int_set

int_set_delete::Int->DIS.IntSet->DIS.IntSet
int_set_delete key int_set=if DIS.member key int_set then DIS.delete key int_set else EE.empty_error
...
hash_set_insert key hash_set=if DHS.member key hash_set then EE.empty_error else DHS.insert key hash_set

hash_set_delete::(Eq a,DH.Hashable a)=>a->DHS.HashSet a->DHS.HashSet a
hash_set_delete key hash_set=if DHS.member key hash_set then DHS.delete key hash_set else EE.empty_error
