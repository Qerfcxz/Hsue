...
create_engine::ET.Has_call_stack=>Custom_state a->(Event a->Engine a->Maybe Int)->(Event a->Engine a->Projection_strategy)->FCT.CFloat->FCT.CFloat->FCT.CInt->DW.Word32->DW.Word32->DW.Word32->DW.Word32->DW.Word32->DW.Word32->DW.Word64->Maybe DW.Word64->Int->Int->Int->Int->Int->Int->Sampler_create_info->Blend_state->Bool->Bool->IO (Engine a)
create_engine engine_custom main_id projection_strategy font_size pixel_range max_picture_size max_vertex_size max_index_size max_parameter_size padding width height time maybe_interval count initial_canvas_id initial_album_id initial_font_id exponent_width exponent_height sampler_create_info blend_state strict_exist strict_match=do
    ...
    let album_id=initial_album_id+1 in case maybe_interval of
        Nothing->return (Engine {engine_custom=engine_custom,main_id=main_id,projection_strategy=projection_strategy,callback=callback,device=device,texture=texture,sampler=DIM.empty,default_sampler=sampler,canvas_graphics_pipeline=canvas_graphics_pipeline,pipeline=DIM.empty,shader=DIM.empty,default_shader=default_shader,vertex_shader=vertex_shader,fragment_shader=fragment_shader,vertex_buffer=vertex_buffer,index_buffer=index_buffer,parameter_buffer=parameter_buffer,transfer_buffer=transfer_buffer,picture_transfer_buffer=picture_transfer_buffer,atlas=atlas,canvas=DIM.empty,album=DIM.singleton initial_album_id (Album {width=width,height=height,texture=new_texture}),leaf=DIM.empty,node=DIM.empty,window=DIM.empty,font=DIM.empty,atlas_font=DIM.empty,window_map=DHMS.empty,font_map=DHMS.empty,system_cursor_map=DHMS.insert System_cursor_pointer system_cursor_pointer (DHMS.singleton System_cursor_default system_cursor_default),request=DS.empty,key=DHS.empty,u=scaleFloat (negate exponent_width) (fromIntegral (left+right)/2),v=scaleFloat (negate exponent_height) (fromIntegral (down+up)/2),font_size=font_size,pixel_range=pixel_range,max_picture_size=max_picture_size,max_vertex_size=max_vertex_size,max_index_size=max_index_size,max_parameter_size=max_parameter_size,padding=padding,event_number=event_number,time=time,timer=Off,count=count,initial_canvas_id=initial_canvas_id,canvas_id=initial_canvas_id,initial_album_id=initial_album_id,album_id=album_id,initial_font_id=initial_font_id,font_id=initial_font_id,exponent_width=exponent_width,exponent_height=exponent_height,strict_exist=strict_exist,strict_match=strict_match})
        Just interval->if 0<interval
            then do
                timer_id<-SDLF.sdl_add_timer_ns interval callback FP.nullPtr
                sdl_catch_zero timer_id
                return (Engine {engine_custom=engine_custom,main_id=main_id,projection_strategy=projection_strategy,callback=callback,device=device,texture=texture,sampler=DIM.empty,default_sampler=sampler,canvas_graphics_pipeline=canvas_graphics_pipeline,pipeline=DIM.empty,shader=DIM.empty,default_shader=default_shader,vertex_shader=vertex_shader,fragment_shader=fragment_shader,vertex_buffer=vertex_buffer,index_buffer=index_buffer,parameter_buffer=parameter_buffer,transfer_buffer=transfer_buffer,picture_transfer_buffer=picture_transfer_buffer,atlas=atlas,canvas=DIM.empty,album=DIM.singleton initial_album_id (Album {width=width,height=height,texture=new_texture}),leaf=DIM.empty,node=DIM.empty,window=DIM.empty,font=DIM.empty,atlas_font=DIM.empty,window_map=DHMS.empty,font_map=DHMS.empty,system_cursor_map=DHMS.insert System_cursor_pointer system_cursor_pointer (DHMS.singleton System_cursor_default system_cursor_default),request=DS.empty,key=DHS.empty,u=scaleFloat (negate exponent_width) (fromIntegral (left+right)/2),v=scaleFloat (negate exponent_height) (fromIntegral (down+up)/2),font_size=font_size,pixel_range=pixel_range,max_picture_size=max_picture_size,max_vertex_size=max_vertex_size,max_index_size=max_index_size,max_parameter_size=max_parameter_size,padding=padding,event_number=event_number,time=time,timer=On {timer_id=timer_id,interval=interval},count=count,initial_canvas_id=initial_canvas_id,canvas_id=initial_canvas_id,initial_album_id=initial_album_id,album_id=album_id,initial_font_id=initial_font_id,font_id=initial_font_id,exponent_width=exponent_width,exponent_height=exponent_height,strict_exist=strict_exist,strict_match=strict_match})
            else EF.empty_error
