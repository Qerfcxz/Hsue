...
create_engine timer main_id state=case timer of
    Nothing->Engine {state=state,leaf=DIM.empty,node=DIM.empty,window=DIM.empty,window_id_map=DM.empty,request=DS.empty,main_id=main_id,timer=Keep_off}
    Just time->Engine {state=state,leaf=DIM.empty,node=DIM.empty,window=DIM.empty,window_id_map=DM.empty,request=DS.empty,main_id=main_id,timer=Keep_on {time=fromIntegral time}}
...
loop_engine ptr engine=do
    ...
    case event of
        Quit->return ()
        _->let new_new_engine=run_event event new_engine in case new_new_engine.timer of
...
loop_engine_time time next_time ptr engine=do
    ...
    if now<next_time
        then do
            ...
            case event of
                Quit->return ()
                Time->loop_engine_time_a (next_time+time) ptr (run_event Time new_engine)
                _->loop_engine_time_a next_time ptr (run_event event new_engine)
        else loop_engine_time_a (next_time+time) ptr (run_event Time new_engine)
    ...
    Just leaf->case leaf.father of
        ...
        Just node_id->case DIM.lookup node_id cache of
            Nothing->let engine_node=engine.node in case DIM.lookup node_id engine_node of
                ...
                Just node->let (seq_node_id,new_event)=run_event_c cache node.ancestry (DS.singleton node_id) event in let (new_cache,new_new_event)=run_event_d engine engine_node seq_node_id cache new_event in run_event_b leaf new_cache new_new_event event engine
...
run_event_c::DIM.IntMap Event->DS.Seq Int->DS.Seq Int->Event->(DS.Seq Int,Event)
run_event_c _ DS.Empty seq_node_id event=(seq_node_id,event)
run_event_c cache (other_node_id DS.:|> node_id) seq_node_id event=let maybe_event=DIM.lookup node_id cache in case maybe_event of
    Nothing->run_event_c cache other_node_id (node_id DS.<| seq_node_id) event
    Just new_event->(seq_node_id,new_event)

run_event_d::Engine a->DIM.IntMap (Node a)->DS.Seq Int->DIM.IntMap Event->Event->(DIM.IntMap Event,Event)
run_event_d _ _ DS.Empty cache event=(cache,event)
run_event_d engine engine_node (node_id DS.:<| other_node_id) cache event=case DIM.lookup node_id engine_node of
