...
create_engine state main_id projection_strategy picture_size vertex_size index_size parameter_size album_id count maybe_interval time padding width height font_size pixel_range=do
    ...
    let new_album_id=album_id+1 in case maybe_interval of
        Nothing->let reciprocal_width=1/fromIntegral width in let reciprocal_height=1/fromIntegral height in return (Engine {state=state,main_id=main_id,projection_strategy=projection_strategy,callback=callback,font=DIM.empty,atlas=new_atlas,album=DIM.singleton album_id (Album {width=new_width,height=new_height,texture=new_texture}),leaf=DIM.empty,node=DIM.empty,window=DIM.empty,window_map=DM.empty,request=DSeq.empty,key=DSet.empty,device=device,texture=texture,sampler=sampler,vertex_shader=vertex_shader,fragment_shader=fragment_shader,vertex_buffer=vertex_buffer,index_buffer=index_buffer,parameter_buffer=parameter_buffer,transfer_buffer=transfer_buffer,picture_transfer_buffer=picture_transfer_buffer,picture_size=picture_size,vertex_size=vertex_size,index_size=index_size,parameter_size=parameter_size,initial_album_id=new_album_id,album_id=new_album_id,count=count,timer=Off,time=time,event_number=event_number,padding=padding,width=width,height=height,reciprocal_width=reciprocal_width,reciprocal_height=reciprocal_height,u=fromIntegral (left+right)*reciprocal_width/2,v=fromIntegral (down+up)*reciprocal_height/2,font_size=font_size,pixel_range=pixel_range})
...
run_event_b projection event engine=case projection of
    Without {ancestry_id}->let this_event=DF.foldl' (\this_this_event node_id->(intmap_lookup node_id engine.node).event_transform engine this_this_event) event ancestry_id in let (this_update,this_next,widget)=run_widget (lookup_projection_object projection) this_event engine in ((`this_next` this_event),this_update,insert_projection_object widget projection)
    With {ancestry_id}->let this_event=DF.foldl' (\this_this_event node_id->(intmap_lookup node_id engine.node).event_transform engine this_this_event) event ancestry_id in let (this_update,this_next,widget)=run_widget (lookup_projection_object projection) this_event engine in ((`this_next` this_event),this_update,insert_projection_object widget projection)
