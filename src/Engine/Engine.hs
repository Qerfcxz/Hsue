...
run_widget this_widget event engine=case this_widget of
    ...
    Coroutine {index,coroutine_state,linear_coroutine}->let (update,next,new_coroutine_state)=intmap_functor_update index (functor_update_coroutine_state (\widget->run_widget widget event engine)) coroutine_state in (update,next,Coroutine {index=index,coroutine_state=new_coroutine_state,linear_coroutine=linear_coroutine})
