...
from_coroutine coroutine int=CMST.runST $ do
    ...
    (linear_coroutine,new_layout,int_index,user_int_index,code_index)<-from_coroutine_a coroutine 1 0 0 0 Nothing layout DIM.empty
...
from_coroutine_a::Coroutine a b c d e->Int->Int->Int->Int->Maybe (Int,Int)->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int)
from_coroutine_a this_coroutine clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine=case this_coroutine of
    ...
    Forever {coroutine}->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine
        case maybe_poison_layout of
            Nothing->return (DIM.insert new_code_index (Linear_jump {code_index=code_index}) new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index+1)
            Just (poison_int_index,poison_clone_number)->return (DIM.insert (new_code_index+1) (Linear_jump {code_index=code_index}) (DIM.insert new_code_index (Linear_alive {int_index=poison_int_index,clone_number=poison_clone_number}) new_linear_coroutine),new_layout,new_int_index,new_user_int_index,new_code_index+2)
    Then {coroutine_sequence}->then_coroutine coroutine_sequence clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine
    While {dynamic_bool,coroutine}->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number (code_index+1) user_int_index int_index maybe_poison_layout layout linear_coroutine
        case maybe_poison_layout of
            Nothing->let end_code_index=new_code_index+1 in return (DIM.insert new_code_index (Linear_jump {code_index=code_index}) (DIM.insert code_index (Linear_false_jump {code_index=end_code_index,dynamic_bool=dynamic_bool}) new_linear_coroutine),new_layout,new_int_index,new_user_int_index,end_code_index)
            Just (poison_int_index,poison_clone_number)->let end_code_index=new_code_index+2 in return (DIM.insert (new_code_index+1) (Linear_jump {code_index=code_index}) (DIM.insert new_code_index (Linear_alive {int_index=poison_int_index,clone_number=poison_clone_number}) (DIM.insert code_index (Linear_false_jump {code_index=end_code_index,dynamic_bool=dynamic_bool}) new_linear_coroutine)),new_layout,new_int_index,new_user_int_index,end_code_index)
    Pause {dynamic_bool,coroutine}->let body_code_index=code_index+2 in let countdown_int_index=int_index+clone_number in do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number body_code_index user_int_index (int_index+2*clone_number) (Just (int_index,clone_number)) layout linear_coroutine
    ...
    Repeat {dynamic_int,coroutine}->let loop_code_index=code_index+2 in do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number loop_code_index user_int_index (int_index+clone_number) maybe_poison_layout layout linear_coroutine
        case maybe_poison_layout of
            Nothing->let end_code_index=new_code_index+1 in return (DIM.insert new_code_index (Linear_one_more_jump {int_index=int_index,code_index=loop_code_index}) (DIM.insert (code_index+1) (Linear_one_less_jump {int_index=int_index,code_index=end_code_index}) (DIM.insert code_index (Linear_dynamic_int {int_index=int_index,dynamic_int=dynamic_int}) new_linear_coroutine)),new_layout,new_int_index,new_user_int_index,end_code_index)
            Just (poison_int_index,poison_clone_number)->let end_code_index=new_code_index+2 in return (DIM.insert (new_code_index+1) (Linear_one_more_jump {int_index=int_index,code_index=loop_code_index}) (DIM.insert new_code_index (Linear_alive {int_index=poison_int_index,clone_number=poison_clone_number}) (DIM.insert (code_index+1) (Linear_one_less_jump {int_index=int_index,code_index=end_code_index}) (DIM.insert code_index (Linear_dynamic_int {int_index=int_index,dynamic_int=dynamic_int}) new_linear_coroutine))),new_layout,new_int_index,new_user_int_index,end_code_index)
    Clone {int,coroutine}->let new_int=int-1 in if new_int<0 then return (linear_coroutine,layout,int_index,user_int_index,code_index) else do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine (int*clone_number) (code_index+1) user_int_index (int_index+clone_number) maybe_poison_layout layout linear_coroutine
    ...
    If {dynamic_bool,first_coroutine,second_coroutine}->do
        (first_linear_coroutine,first_layout,first_int_index,first_user_int_index,first_code_index)<-from_coroutine_a first_coroutine clone_number (code_index+1) user_int_index int_index maybe_poison_layout layout linear_coroutine
        let new_code_index=first_code_index+1
        (second_linear_coroutine,second_layout,second_int_index,second_user_int_index,second_code_index)<-from_coroutine_a second_coroutine clone_number new_code_index first_user_int_index int_index maybe_poison_layout first_layout first_linear_coroutine
    ...
    Case {dynamic_int,int,coroutine_sequence}->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,single_linear_coroutine)<-case_coroutine DIM.empty DIS.empty coroutine_sequence int 0 (int_index+clone_number) clone_number (code_index+2) user_int_index int_index maybe_poison_layout layout linear_coroutine
    ...
    Fork {int,coroutine,coroutine_sequence}->let body_code_index=code_index+1 in do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number (body_code_index+int) user_int_index (int_index+clone_number) maybe_poison_layout layout linear_coroutine
        (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index)<-fork_coroutine DIS.empty coroutine_sequence body_code_index clone_number (new_code_index+1) new_user_int_index new_int_index maybe_poison_layout new_layout (DIM.insert code_index (Linear_int {int_index=int_index,int=int}) new_linear_coroutine)
    ...
    Race {dynamic_int,first_int,second_int,coroutine_sequence}->let countdown_int_index=int_index+clone_number in do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_code_index)<-race_coroutine coroutine_sequence 0 countdown_int_index clone_number (code_index+2) user_int_index (int_index+2*clone_number) (Just (int_index,clone_number)) layout linear_coroutine
...
then_coroutine::DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->Maybe (Int,Int)->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int)
then_coroutine coroutine_sequence clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine=case coroutine_sequence of
    DS.Empty->return (linear_coroutine,layout,int_index,user_int_index,code_index)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine
        then_coroutine other_coroutine clone_number new_code_index new_user_int_index new_int_index maybe_poison_layout new_layout new_linear_coroutine

case_coroutine::DIM.IntMap Int->DIS.IntSet->DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->Int->Int->Int->Maybe (Int,Int)->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int,Linear_coroutine a b c d e)
case_coroutine case_code_index jump_code_index coroutine_sequence int index max_int_index clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine=case coroutine_sequence of
    ...
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index (int_index+clone_number) maybe_poison_layout layout linear_coroutine
        case_coroutine (DIM.insert index code_index case_code_index) (DIS.insert new_code_index jump_code_index) other_coroutine int (index+1) (max new_int_index max_int_index) clone_number (new_code_index+1) (max user_int_index new_user_int_index) int_index maybe_poison_layout new_layout new_linear_coroutine
...
fork_coroutine::DIS.IntSet->DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->Int->Maybe (Int,Int)->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int)
fork_coroutine jump_code_index coroutine_sequence fork_code_index clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine=case coroutine_sequence of
    ...
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine
        fork_coroutine (DIS.insert (code_index-1) jump_code_index) other_coroutine (fork_code_index+1) clone_number (new_code_index+1) new_user_int_index new_int_index maybe_poison_layout new_layout (DIM.insert fork_code_index (Linear_fork {code_index=code_index}) new_linear_coroutine)

race_coroutine::DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->Int->Int->Maybe (Int,Int)->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int,DIM.IntMap Int)
race_coroutine coroutine_sequence index countdown_int_index clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine=case coroutine_sequence of
    DS.Empty->return (linear_coroutine,layout,int_index,user_int_index,code_index,DIM.empty)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index maybe_poison_layout layout linear_coroutine
        (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,group_code_index)<-race_coroutine other_coroutine (index+1) countdown_int_index clone_number (new_code_index+1) new_user_int_index (max int_index new_int_index) maybe_poison_layout new_layout (DIM.insert new_code_index (Linear_countdown {int_index=countdown_int_index}) new_linear_coroutine)
...
step_coroutine_a linear_coroutine main_index clone_index this_code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget=case linear_coroutine DV.! this_code_index of
    ...
    Linear_alive {int_index,clone_number}->do
        int<-DVUM.read variable (int_index+mod clone_index clone_number)
        if int==(-1) then step_coroutine linear_coroutine program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget else step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget
