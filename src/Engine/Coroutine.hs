...
run_coroutine_b event engine iterative linear_coroutine layout coroutine_state update=case coroutine_state of
...
step_coroutine_a linear_coroutine main_index clone_index this_code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget=case linear_coroutine DV.! this_code_index of
    ...
    Linear_assign {user_int_index,clone_number,dynamic_int}->let new_value=dynamic_int.dynamic_int (user_variable_getter user_variable layout clone_index) event engine widget in do
        new_user_variable<-DVU.unsafeThaw user_variable
        DVUM.write new_user_variable (user_int_index+mod clone_index clone_number) new_value
...
run_clone int clone_number clone_index code_index program_counter_index main_index_group program_counter=if int<1 then (program_counter,main_index_group,program_counter_index) else run_clone (int-1) clone_number (clone_index+clone_number) code_index (program_counter_index+1) (main_index_group DS.|> program_counter_index) (int_map_insert program_counter_index (Program_counter {code_index=code_index,clone_index=clone_index}) program_counter)
...
run_create_group group_code_index int index clone_index program_counter_index index_group_index index_group program_counter=if int<=index then (program_counter,index_group,program_counter_index) else run_create_group group_code_index int (index+1) clone_index (program_counter_index+1) index_group_index (int_map_insert (index_group_index+index) (DS.singleton program_counter_index) index_group) (int_map_insert program_counter_index (Program_counter {code_index=int_map_lookup index group_code_index,clone_index=clone_index}) program_counter)
