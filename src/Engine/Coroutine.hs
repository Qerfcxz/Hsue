...
from_coroutine coroutine layout_size=CMST.runST $ do
    ...
    return (DV.generate (code_index+1) (\this_code_index->if code_index==this_code_index then Linear_end else int_map_lookup this_code_index linear_coroutine),new_new_layout,int_index,user_int_index)
