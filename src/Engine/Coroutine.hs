...
run_clone int clone_number clone_index code_index program_counter_index main_index_group program_counter=if int<1 then (program_counter,main_index_group,program_counter_index) else run_clone (int-1) clone_number (clone_index+clone_number) code_index (program_counter_index+1) (main_index_group DS.|> program_counter_index) (DIM.insert program_counter_index (Program_counter {code_index=code_index,clone_index=clone_index}) program_counter)
...
run_create_group group_code_index int index clone_index program_counter_index index_group_index index_group program_counter=if int<=index then (program_counter,index_group,program_counter_index) else run_create_group group_code_index int (index+1) clone_index (program_counter_index+1) index_group_index (DIM.insert (index_group_index+index) (DS.singleton program_counter_index) index_group) (DIM.insert program_counter_index (Program_counter {code_index=int_map_lookup index group_code_index,clone_index=clone_index}) program_counter)
...
run_create_active_group group_code_index int index clone_index program_counter_index index_group_index newborn_main_index_group index_group program_counter=if int<=index then (program_counter,index_group,program_counter_index,newborn_main_index_group) else run_create_active_group group_code_index int (index+1) clone_index (program_counter_index+1) index_group_index (newborn_main_index_group DS.|> program_counter_index) (DIM.insert (index_group_index+index) (DS.singleton program_counter_index) index_group) (DIM.insert program_counter_index (Program_counter {code_index=int_map_lookup index group_code_index,clone_index=clone_index}) program_counter)

{-# INLINE run_coroutine #-}
{-# INLINE run_coroutine_a #-}
{-# INLINE run_coroutine_b #-}
{-# INLINE run_coroutine_c #-}
{-# INLINE run_coroutine_d #-}
