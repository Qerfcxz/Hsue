...
run_coroutine_a this_index event engine widget=case widget of
    Coroutine {initial_min_index,min_index,initial_max_index,max_index,index,variable_size,user_variable_size,coroutine_state,layout,linear_coroutine,iterative}->let (update,new_coroutine_state)=DF.foldl' (\(this_update,this_coroutine_state) single_index->if DIM.member single_index this_coroutine_state then int_map_functor_update single_index (\single_coroutine_state->run_coroutine_b iterative linear_coroutine layout single_coroutine_state event engine this_update) this_coroutine_state else if engine.strict_exist then EF.empty_error else (this_update,this_coroutine_state)) (DS.empty,coroutine_state) this_index in (Coroutine {initial_min_index=initial_min_index,min_index=min_index,initial_max_index=initial_max_index,max_index=max_index,index=index,variable_size=variable_size,user_variable_size=user_variable_size,coroutine_state=new_coroutine_state,layout=layout,linear_coroutine=linear_coroutine,iterative=iterative},update)
    _->if engine.strict_match then EF.empty_error else (widget,DS.empty)

run_coroutine_b::ET.Has_call_stack=>Bool->DV.Vector (Linear_coroutine a)->DVS.Vector Layout->Coroutine_state a->Event a->Engine a->DS.Seq (Engine a->Engine a)->(DS.Seq (Engine a->Engine a),Coroutine_state a)
run_coroutine_b iterative linear_coroutine layout coroutine_state event engine update=case coroutine_state of
    Coroutine_state {widget,variable,user_variable,program_counter,index_group,main_index_group,index_group_index,program_counter_index}->CMST.runST $ do
        ...
        (new_widget,_,new_update,new_user_variable,new_program_counter,new_index_group,new_main_index_group,new_index_group_index,new_program_counter_index)<-step_coroutine linear_coroutine program_counter_index index_group_index DS.empty DS.empty main_index_group index_group program_counter layout user_variable new_variable (for_iterative iterative) DS.empty event (if iterative then DF.foldl' (\this_engine this_update->this_update this_engine) engine update else engine) widget
        new_new_variable<-DVU.unsafeFreeze new_variable
        return (update DS.>< new_update,Coroutine_state {widget=new_widget,variable=new_new_variable,user_variable=new_user_variable,program_counter=new_program_counter,index_group=new_index_group,main_index_group=new_main_index_group,index_group_index=new_index_group_index,program_counter_index=new_program_counter_index})

for_iterative::ET.Has_call_stack=>Bool->(Engine a->Engine a)->Engine a->Engine a
for_iterative iterative update engine=if iterative then update engine else engine

init_coroutine_state::ET.Has_call_stack=>Int->Int->Widget a->Coroutine_state a
...
to_coroutine::ET.Has_call_stack=>DS.Seq (Coroutine a)->Coroutine a
...
lift_coroutine::ET.Has_call_stack=>Coroutine a->Raw_coroutine a ()
lift_coroutine coroutine=Raw_coroutine {iterator=lift_coroutine_a coroutine}

lift_coroutine_a::ET.Has_call_stack=>Coroutine a->Int->(Int,DS.Seq (Coroutine a),())
lift_coroutine_a coroutine int=(int,DS.singleton coroutine,())

do_empty::ET.Has_call_stack=>Raw_coroutine a ()
do_empty=Raw_coroutine {iterator=do_empty_a}

do_empty_a::ET.Has_call_stack=>Int->(Int,DS.Seq (Coroutine a),())
do_empty_a int=(int,DS.empty,())

do_declare::ET.Has_call_stack=>Raw_coroutine a Int
do_declare=Raw_coroutine {iterator=do_declare_a}

do_declare_a::ET.Has_call_stack=>Int->(Int,DS.Seq (Coroutine a),Int)
do_declare_a int=(int+1,DS.empty,int)

do_emit::ET.Has_call_stack=>(Event a->Engine a->Widget a->(Widget a,Engine a->Engine a))->Raw_coroutine a ()
do_emit emit=lift_coroutine (Emit {emit=emit})

do_wait::ET.Has_call_stack=>Dynamic_int a->Raw_coroutine a ()
do_wait dynamic_int=lift_coroutine (Wait {dynamic_int=dynamic_int})

do_forever::ET.Has_call_stack=>Raw_coroutine a ()->Raw_coroutine a ()
...
do_then::ET.Has_call_stack=>Raw_coroutine a ()->Raw_coroutine a ()
...
do_while::ET.Has_call_stack=>Dynamic_bool a->Raw_coroutine a ()->Raw_coroutine a ()
...
do_pause::ET.Has_call_stack=>Dynamic_bool a->Raw_coroutine a ()->Raw_coroutine a ()
...
do_skip::ET.Has_call_stack=>Dynamic_bool a->Raw_coroutine a ()->Raw_coroutine a ()
...
do_assign::ET.Has_call_stack=>Int->Dynamic_int a->Raw_coroutine a ()
do_assign int dynamic_int=lift_coroutine (Assign {int=int,dynamic_int=dynamic_int})

do_repeat::ET.Has_call_stack=>Dynamic_int a->Raw_coroutine a ()->Raw_coroutine a ()
...
do_clone::ET.Has_call_stack=>Int->Raw_coroutine a ()->Raw_coroutine a ()
...
do_if::ET.Has_call_stack=>Dynamic_bool a->Raw_coroutine a ()->Raw_coroutine a ()->Raw_coroutine a ()
...
do_dynamic_clone::ET.Has_call_stack=>Int->Dynamic_int a->Raw_coroutine a ()->Raw_coroutine a ()
do_dynamic_clone int dynamic_int raw_coroutine=Raw_coroutine {iterator=raw_coroutine_unary_operator (\coroutine_sequence->DS.singleton (Dynamic_clone {int=int,dynamic_int=dynamic_int,coroutine=to_coroutine coroutine_sequence})) raw_coroutine.iterator}

do_case::ET.Has_call_stack=>Dynamic_int a->Raw_coroutine a ()->Raw_coroutine a ()
do_case dynamic_int raw_coroutine=Raw_coroutine {iterator=raw_coroutine_unary_generator (\coroutine other_coroutine coroutine_sequence->let int=DS.length other_coroutine in if int==0 then DS.singleton coroutine else DS.singleton (Case {int=int+1,dynamic_int=dynamic_int,coroutine_sequence=coroutine_sequence})) raw_coroutine.iterator}

do_fork::ET.Has_call_stack=>Raw_coroutine a ()->Raw_coroutine a ()
...
do_race::ET.Has_call_stack=>Int->Dynamic_int a->Raw_coroutine a ()->Raw_coroutine a ()
do_race int dynamic_int raw_coroutine=Raw_coroutine {iterator=raw_coroutine_unary_generator (\coroutine other_coroutine coroutine_sequence->let new_int=DS.length other_coroutine in if new_int==0 then DS.singleton coroutine else DS.singleton (Race {first_int=int,second_int=new_int+1,dynamic_int=dynamic_int,coroutine_sequence=coroutine_sequence})) raw_coroutine.iterator}

raw_coroutine_unary_generator::ET.Has_call_stack=>(Coroutine a->DS.Seq (Coroutine a)->DS.Seq (Coroutine a)->DS.Seq (Coroutine a))->(Int->(Int,DS.Seq (Coroutine a),()))->Int->(Int,DS.Seq (Coroutine a),())
...
raw_coroutine_unary_operator::ET.Has_call_stack=>(DS.Seq (Coroutine a)->DS.Seq (Coroutine a))->(Int->(Int,DS.Seq (Coroutine a),()))->Int->(Int,DS.Seq (Coroutine a),())
...
raw_coroutine_binary_operator::ET.Has_call_stack=>(DS.Seq (Coroutine a)->DS.Seq (Coroutine a)->DS.Seq (Coroutine a))->(Int->(Int,DS.Seq (Coroutine a),()))->(Int->(Int,DS.Seq (Coroutine a),()))->Int->(Int,DS.Seq (Coroutine a),())
...
from_coroutine::ET.Has_call_stack=>Coroutine a->Int->(DV.Vector (Linear_coroutine a),DVS.Vector Layout,Int,Int)
from_coroutine coroutine layout_size=CMST.runST $ do
    ...
    return (DV.generate (code_index+1) (\this_code_index->if code_index==this_code_index then Linear_done else int_map_lookup this_code_index linear_coroutine),new_new_layout,int_index,user_int_index)

from_coroutine_a::ET.Has_call_stack=>Coroutine a->Int->Int->Int->Int->Int->DVSM.MVector b Layout->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVSM.MVector b Layout,Int,Int,Int,DIM.IntMap Int)
from_coroutine_a this_coroutine layout_size clone_number code_index user_int_index int_index layout linear_coroutine=case this_coroutine of
    ...
    Clone {int,coroutine}->if int<=0 then EF.empty_error else do
        ...
        return (DIM.insert new_code_index (Linear_kill_clone {int_index=int_index,clone_number=clone_number}) (DIM.insert code_index (Linear_clone {int=int-1,int_index=int_index,clone_number=clone_number}) new_linear_coroutine),new_layout,new_int_index,new_user_int_index,new_code_index+1,clone_coroutine group_int_index int clone_number)
    ...
    Dynamic_clone {int,dynamic_int,coroutine}->if int<=0 then EF.empty_error else do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine layout_size (int*clone_number) (code_index+1) user_int_index (int_index+clone_number) layout linear_coroutine
        let kill_code_index=new_code_index+1 in return (DIM.insert new_code_index (Linear_kill_clone {int_index=int_index,clone_number=clone_number}) (DIM.insert code_index (Linear_dynamic_clone {int=int-1,int_index=int_index,code_index=kill_code_index,clone_number=clone_number,dynamic_int=dynamic_int}) new_linear_coroutine),new_layout,new_int_index,new_user_int_index,kill_code_index,clone_coroutine group_int_index int clone_number)
    Case {int,dynamic_int,coroutine_sequence}->do
    ...
    Fork {int,coroutine,coroutine_sequence}->if int<=0 then EF.empty_error else do
        ...
        let kill_code_index=new_code_index+2
        ...
        let all_group_int_index=DIM.insert int_index int (DIM.union main_group_int_index fork_group_int_index) in return (DIM.insert jump_code_index (Linear_jump {code_index=fork_end_code_index}) (DIM.insert kill_code_index (Linear_kill {group_int_index=all_group_int_index}) (DIM.insert (new_code_index+1) (Linear_yield {code_index=new_code_index}) (DIM.insert new_code_index (Linear_less_jump {int=1,int_index=countdown_int_index,code_index=kill_code_index}) (DIM.insert (code_index+1) (Linear_jump {code_index=code_index+2}) (DIM.insert code_index (Linear_create_active_group {int=int,first_int_index=int_index,second_int_index=countdown_int_index,group_code_index=group_code_index}) final_linear_coroutine))))),final_layout,final_int_index,final_user_int_index,fork_end_code_index,all_group_int_index)
    Race {first_int,second_int,dynamic_int,coroutine_sequence}->let countdown_int_index=int_index+clone_number in do
        ...
        let kill_code_index=new_code_index+3 in let new_group_int_index=DIM.insert int_index second_int group_int_index in return (DIM.insert kill_code_index (Linear_kill {group_int_index=new_group_int_index}) (DIM.insert (new_code_index+2) (Linear_yield {code_index=new_code_index}) (DIM.insert (new_code_index+1) (Linear_wake_group {int_index=int_index,dynamic_int=dynamic_int}) (DIM.insert new_code_index (Linear_less_jump {int=second_int-first_int+1,int_index=countdown_int_index,code_index=kill_code_index}) (DIM.insert (code_index+1) (Linear_jump {code_index=new_code_index}) (DIM.insert code_index (Linear_create_group {int=second_int,first_int_index=int_index,second_int_index=countdown_int_index,group_code_index=group_code_index}) new_linear_coroutine))))),new_layout,new_int_index,new_user_int_index,new_code_index+4,new_group_int_index)

then_coroutine::ET.Has_call_stack=>DS.Seq (Coroutine a)->Int->Int->Int->Int->Int->DVSM.MVector b Layout->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVSM.MVector b Layout,Int,Int,Int,DIM.IntMap Int)
...
clone_coroutine::ET.Has_call_stack=>DIM.IntMap Int->Int->Int->DIM.IntMap Int
...
clone_coroutine_a::ET.Has_call_stack=>Int->Int->Int->Int->Int->DIM.IntMap Int->DIM.IntMap Int
clone_coroutine_a int index int_index clone_number size group_int_index=if int<=index then group_int_index else clone_coroutine_a int (index+1) int_index clone_number size (DIM.insert (int_index+index*clone_number) size group_int_index)

case_coroutine::ET.Has_call_stack=>DIM.IntMap Int->DIS.IntSet->DS.Seq (Coroutine a)->Int->Int->Int->Int->Int->Int->Int->Int->DVSM.MVector b Layout->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVSM.MVector b Layout,Int,Int,Int,Linear_coroutine a,DIM.IntMap Int)
...
case_coroutine_a::ET.Has_call_stack=>DIM.IntMap Int->Int->Int->Int->Int->DIM.IntMap (Linear_coroutine a)->(DIM.IntMap (Linear_coroutine a),Int)
case_coroutine_a case_code_index int index code_index int_index linear_coroutine=case int of
    1->(DIM.insert code_index (Linear_jump {code_index=int_map_lookup index case_code_index}) linear_coroutine,code_index+1)
    _->let half_int=div int 2 in let new_int=index+half_int in let (right_linear_coroutine,right_code_index)=case_coroutine_a case_code_index (int-half_int) new_int (code_index+1) int_index linear_coroutine in let (left_linear_coroutine,left_code_index)=case_coroutine_a case_code_index half_int index right_code_index int_index right_linear_coroutine in (DIM.insert code_index (Linear_less_jump {int=new_int,int_index=int_index,code_index=right_code_index}) left_linear_coroutine,left_code_index)

fork_coroutine::ET.Has_call_stack=>DS.Seq (Coroutine a)->Int->Int->Int->Int->Int->Int->Int->DVSM.MVector b Layout->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVSM.MVector b Layout,Int,Int,Int,DIM.IntMap Int,DIM.IntMap Int)
...
race_coroutine::ET.Has_call_stack=>DS.Seq (Coroutine a)->Int->Int->Int->Int->Int->Int->Int->DVSM.MVector b Layout->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVSM.MVector b Layout,Int,Int,Int,DIM.IntMap Int,DIM.IntMap Int)
...
step_coroutine::ET.Has_call_stack=>DV.Vector (Linear_coroutine a)->Int->Int->DS.Seq Int->DS.Seq Int->DS.Seq Int->DIM.IntMap (DS.Seq Int)->DIM.IntMap Program_counter->DVS.Vector Layout->DVU.Vector Int->DVUM.MVector b Int->((Engine a->Engine a)->Engine a->Engine a)->DS.Seq (Engine a->Engine a)->Event a->Engine a->Widget a->CMST.ST b (Widget a,Engine a,DS.Seq (Engine a->Engine a),DVU.Vector Int,DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int),DS.Seq Int,Int,Int)
step_coroutine linear_coroutine program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget=case newborn_main_index_group of
    DS.Empty->case main_index_group of
        DS.Empty->return (widget,engine,update,user_variable,program_counter,index_group,survived_main_index_group,index_group_index,program_counter_index)
...
step_coroutine_a::ET.Has_call_stack=>DV.Vector (Linear_coroutine a)->Int->Int->Int->Int->Int->DS.Seq Int->DS.Seq Int->DS.Seq Int->DIM.IntMap (DS.Seq Int)->DIM.IntMap Program_counter->DVS.Vector Layout->DVU.Vector Int->DVUM.MVector b Int->((Engine a->Engine a)->Engine a->Engine a)->DS.Seq (Engine a->Engine a)->Event a->Engine a->Widget a->CMST.ST b (Widget a,Engine a,DS.Seq (Engine a->Engine a),DVU.Vector Int,DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int),DS.Seq Int,Int,Int)
step_coroutine_a linear_coroutine main_index clone_index this_code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget=case linear_coroutine DV.! this_code_index of
    Linear_done->step_coroutine linear_coroutine program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget
    ...
    Linear_wake {int_index}->do
        int<-DVUM.read variable (int_index+clone_index)
        let (new_index_group,maybe_single_index_group)=int_map_delete_maybe_lookup int index_group in case maybe_single_index_group of
    ...
    Linear_kill {group_int_index}->do
        (new_program_counter,new_index_group)<-int_map_monad_fold (\int_index int (this_program_counter,this_index_group)->run_kill_group int int_index clone_index variable this_program_counter this_index_group) group_int_index (program_counter,index_group)
        step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group new_index_group new_program_counter layout user_variable variable updater update event engine widget
    ...
    Linear_less_jump {int,int_index,code_index}->do
        new_int<-DVUM.read variable (int_index+clone_index)
        if new_int<int then step_coroutine_a linear_coroutine main_index clone_index code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget else step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget
    Linear_clone {int,int_index,clone_number}->do
    ...
    Linear_wake_group {int_index,dynamic_int}->do
        int<-DVUM.read variable (int_index+clone_index)
        let new_index_group_index=int+dynamic_int.dynamic_int (user_variable_getter user_variable layout clone_index) event engine widget in let (new_index_group,maybe_single_index_group)=int_map_delete_maybe_lookup new_index_group_index index_group in case maybe_single_index_group of
            Nothing->step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget
            Just single_index_group->do
                (new_widget,new_engine,new_update,new_user_variable,new_program_counter,new_new_index_group,new_survived_main_index_group,new_new_index_group_index,new_program_counter_index)<-step_coroutine linear_coroutine program_counter_index index_group_index DS.empty DS.empty single_index_group new_index_group program_counter layout user_variable variable updater DS.empty event engine widget
                alive<-DVUM.read variable (int_index+clone_index)
    ...
    Linear_create_group {int,first_int_index,second_int_index,group_code_index}->do
    ...
    Linear_create_active_group {int,first_int_index,second_int_index,group_code_index}->do
        DVUM.write variable (first_int_index+clone_index) index_group_index
        DVUM.write variable (second_int_index+clone_index) int
    ...
    Linear_dynamic_clone {int,int_index,code_index,clone_number,dynamic_int}->let new_int=dynamic_int.dynamic_int (user_variable_getter user_variable layout clone_index) event engine widget-1 in if new_int<0 then step_coroutine_a linear_coroutine main_index clone_index code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget else if int<new_int then EF.empty_error else do
...
user_variable_getter::ET.Has_call_stack=>DVU.Vector Int->DVS.Vector Layout->Int->Int->Int
...
run_kill_group::ET.Has_call_stack=>Int->Int->Int->DVUM.MVector a Int->DIM.IntMap Program_counter->DIM.IntMap (DS.Seq Int)->CMST.ST a (DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int))
run_kill_group int int_index clone_index variable program_counter index_group=do
    ...
    if new_int<=0 then return (program_counter,index_group) else do
        DVUM.write variable (int_index+clone_index) (negate 1)
        return (run_kill_group_a int 0 new_int index_group program_counter)

run_kill_group_a::ET.Has_call_stack=>Int->Int->Int->DIM.IntMap (DS.Seq Int)->DIM.IntMap Program_counter->(DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int))
run_kill_group_a int index index_group_index index_group program_counter=if int<=index then (program_counter,index_group) else let (new_index_group,maybe_single_index_group)=int_map_delete_maybe_lookup (index_group_index+index) index_group in case maybe_single_index_group of
...
run_clone::ET.Has_call_stack=>Int->Int->Int->Int->Int->DS.Seq Int->DIM.IntMap Program_counter->(DIM.IntMap Program_counter,DS.Seq Int,Int)
run_clone int clone_number clone_index code_index program_counter_index main_index_group program_counter=if int<1 then (program_counter,main_index_group,program_counter_index) else run_clone (int-1) clone_number (clone_index+clone_number) code_index (program_counter_index+1) (main_index_group DS.|> program_counter_index) (int_map_insert_strict program_counter_index (Program_counter {code_index=code_index,clone_index=clone_index}) program_counter)

run_create_group::ET.Has_call_stack=>DIM.IntMap Int->Int->Int->Int->Int->Int->DIM.IntMap (DS.Seq Int)->DIM.IntMap Program_counter->(DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int),Int)
run_create_group group_code_index int index clone_index program_counter_index index_group_index index_group program_counter=if int<=index then (program_counter,index_group,program_counter_index) else run_create_group group_code_index int (index+1) clone_index (program_counter_index+1) index_group_index (int_map_insert_strict (index_group_index+index) (DS.singleton program_counter_index) index_group) (int_map_insert_strict program_counter_index (Program_counter {code_index=int_map_lookup index group_code_index,clone_index=clone_index}) program_counter)
...
run_create_active_group group_code_index int index clone_index program_counter_index index_group_index newborn_main_index_group index_group program_counter=if int<=index then (program_counter,index_group,program_counter_index,newborn_main_index_group) else run_create_active_group group_code_index int (index+1) clone_index (program_counter_index+1) index_group_index (newborn_main_index_group DS.|> program_counter_index) (int_map_insert_strict (index_group_index+index) (DS.singleton program_counter_index) index_group) (int_map_insert_strict program_counter_index (Program_counter {code_index=int_map_lookup index group_code_index,clone_index=clone_index}) program_counter)
