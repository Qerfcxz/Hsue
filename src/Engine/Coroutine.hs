...
run_coroutine_a this_index event engine widget=case widget of
    Coroutine {index,initial_min_index,min_index,initial_max_index,max_index,variable_length,user_variable_length,coroutine_state,address,size,linear_coroutine,iterative}->let (update,new_coroutine_state)=DF.foldl' (\(this_update,this_coroutine_state) this_this_index->intmap_functor_update this_this_index (DT.swap . run_coroutine_b this_update event engine iterative linear_coroutine size address) this_coroutine_state) (DS.empty,coroutine_state) this_index in (Coroutine {index=index,initial_min_index=initial_min_index,min_index=min_index,initial_max_index=initial_max_index,max_index=max_index,variable_length=variable_length,user_variable_length=user_variable_length,coroutine_state=new_coroutine_state,address=address,size=size,linear_coroutine=linear_coroutine,iterative=iterative},update)
    _->EE.quick_error "run_coroutine_a" 0

run_coroutine_b::DS.Seq (Engine a->Engine a)->Event->Engine a->Bool->DV.Vector (Linear_coroutine a)->DVU.Vector Int->DVU.Vector Int->Coroutine_state a->(Coroutine_state a,DS.Seq (Engine a->Engine a))
run_coroutine_b update event engine iterative linear_coroutine size address coroutine_state=case coroutine_state of
    Coroutine_state {widget,variable,user_variable,program_counter,index_group,main_index_group,index_group_index,program_counter_index}->CMST.runST $ do
        ...
        (new_widget,_,new_update,new_new_variable,new_user_variable,new_program_counter,new_index_group,new_main_index_group,new_index_group_index,new_program_counter_index)<-step_coroutine linear_coroutine program_counter_index index_group_index DS.empty DS.empty main_index_group index_group program_counter address size user_variable new_variable (for_iterative iterative) DS.empty event (if iterative then DF.foldl' (\this_engine this_update->this_update this_engine) engine update else engine) widget
...
from_coroutine::Coroutine a->Int->(DV.Vector (Linear_coroutine a),DVU.Vector Int,DVU.Vector Int,Int,Int)
from_coroutine coroutine int=CMST.runST $ do
    address<-DVUM.replicate int 0
    size<-DVUM.replicate int 0
    (linear_coroutine,new_size,new_address,int_index,user_int_index,code_index)<-from_coroutine_a coroutine 1 0 0 0 address size DIM.empty
    new_new_address<-DVU.unsafeFreeze new_address
    new_new_size<-DVU.unsafeFreeze new_size
    return (DV.generate (code_index+1) (\this_code_index->if code_index==this_code_index then Linear_end else intmap_lookup this_code_index linear_coroutine),new_new_size,new_new_address,int_index,user_int_index)

from_coroutine_a::Coroutine a->Int->Int->Int->Int->DVUM.MVector b Int->DVUM.MVector b Int->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVUM.MVector b Int,DVUM.MVector b Int,Int,Int,Int)
from_coroutine_a this_coroutine clone_number code_index user_int_index int_index address size linear_coroutine=case this_coroutine of
    Done->return (linear_coroutine,size,address,int_index,user_int_index,code_index)
    Emit {emit}->return (DIM.insert code_index (Linear_emit {emit=emit}) linear_coroutine,size,address,int_index,user_int_index,code_index+1)
    Wait {dynamic_int}->return (DIM.insert (code_index+1) (Linear_wait {int_index=int_index}) (DIM.insert code_index (Linear_dynamic_int {int_index=int_index,dynamic_int=dynamic_int}) linear_coroutine),size,address,int_index+clone_number,user_int_index,code_index+2)
    Forever {coroutine}->do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index address size linear_coroutine
        return (DIM.insert new_code_index (Linear_jump {code_index=code_index}) new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index+1)
    Then {coroutine_sequence}->then_coroutine coroutine_sequence clone_number code_index user_int_index int_index address size linear_coroutine
    While {dynamic_bool,coroutine}->do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number (code_index+1) user_int_index int_index address size linear_coroutine
        let end_code_index=new_code_index+1 in return (DIM.insert new_code_index (Linear_jump {code_index=code_index}) (DIM.insert code_index (Linear_false_jump {code_index=end_code_index,dynamic_bool=dynamic_bool}) new_linear_coroutine),new_size,new_address,new_int_index,new_user_int_index,end_code_index)
    Pause {dynamic_bool,coroutine}->let body_code_index=code_index+2 in let countdown_int_index=int_index+clone_number in do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number body_code_index user_int_index (int_index+2*clone_number) address size linear_coroutine
        let loop_code_index=new_code_index+1 in let yield_code_index=loop_code_index+3 in let jump_code_index=loop_code_index+4 in return (DIM.insert jump_code_index (Linear_kill_group {int_index=int_index,int=1}) (DIM.insert yield_code_index (Linear_yield {code_index=loop_code_index}) (DIM.insert (loop_code_index+2) (Linear_wake {int_index=int_index}) (DIM.insert (loop_code_index+1) (Linear_true_jump {code_index=yield_code_index,dynamic_bool=dynamic_bool}) (DIM.insert loop_code_index (Linear_less_jump {int_index=countdown_int_index,code_index=jump_code_index,int=1}) (DIM.insert new_code_index (Linear_countdown {int_index=countdown_int_index}) (DIM.insert (code_index+1) (Linear_jump {code_index=loop_code_index}) (DIM.insert code_index (Linear_create_group {first_int_index=int_index,second_int_index=countdown_int_index,group_code_index=DIM.singleton 0 body_code_index,int=1}) new_linear_coroutine))))))),new_size,new_address,new_int_index,new_user_int_index,loop_code_index+5)
    ...
    Assign {dynamic_int,int}->do
        new_clone_number<-DVUM.read size int
        if new_clone_number==0
            then do
                DVUM.write address int user_int_index
                DVUM.write size int clone_number
                return (DIM.insert code_index (Linear_assign {user_int_index=user_int_index,clone_number=clone_number,dynamic_int=dynamic_int}) linear_coroutine,size,address,int_index,user_int_index+clone_number,code_index+1)
            else do
                new_user_int_index<-DVUM.read address int
                return (DIM.insert code_index (Linear_assign {user_int_index=new_user_int_index,clone_number=new_clone_number,dynamic_int=dynamic_int}) linear_coroutine,size,address,int_index,user_int_index,code_index+1)
    Repeat {dynamic_int,coroutine}->let loop_code_index=code_index+2 in do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number loop_code_index user_int_index (int_index+clone_number) address size linear_coroutine
        let end_code_index=new_code_index+1 in return (DIM.insert new_code_index (Linear_one_more_jump {int_index=int_index,code_index=loop_code_index}) (DIM.insert (code_index+1) (Linear_one_less_jump {int_index=int_index,code_index=end_code_index}) (DIM.insert code_index (Linear_dynamic_int {int_index=int_index,dynamic_int=dynamic_int}) new_linear_coroutine)),new_size,new_address,new_int_index,new_user_int_index,end_code_index)
    Clone {int,coroutine}->let new_int=int-1 in if new_int<0 then return (linear_coroutine,size,address,int_index,user_int_index,code_index) else do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine (int*clone_number) (code_index+1) user_int_index (int_index+clone_number) address size linear_coroutine
        return (DIM.insert new_code_index (Linear_kill_clone {int_index=int_index,clone_number=clone_number}) (DIM.insert code_index (Linear_clone {int_index=int_index,clone_number=clone_number,int=new_int}) new_linear_coroutine),new_size,new_address,new_int_index,new_user_int_index,new_code_index+1)
    If {dynamic_bool,first_coroutine,second_coroutine}->do
        (first_linear_coroutine,first_size,first_address,first_int_index,first_user_int_index,first_code_index)<-from_coroutine_a first_coroutine clone_number (code_index+1) user_int_index int_index address size linear_coroutine
        let new_code_index=first_code_index+1
        (second_linear_coroutine,second_size,second_address,second_int_index,second_user_int_index,second_code_index)<-from_coroutine_a second_coroutine clone_number new_code_index first_user_int_index int_index first_address first_size first_linear_coroutine
        return (DIM.insert first_code_index (Linear_jump {code_index=second_code_index}) (DIM.insert code_index (Linear_false_jump {code_index=new_code_index,dynamic_bool=dynamic_bool}) second_linear_coroutine),second_size,second_address,max first_int_index second_int_index,max first_user_int_index second_user_int_index,second_code_index)
    Dynamic_clone {dynamic_int,int,coroutine}->let new_int=int-1 in if new_int<0 then return (linear_coroutine,size,address,int_index,user_int_index,code_index) else do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine (int*clone_number) (code_index+1) user_int_index (int_index+clone_number) address size linear_coroutine
        let kill_code_index=new_code_index+1 in return (DIM.insert new_code_index (Linear_kill_clone {int_index=int_index,clone_number=clone_number}) (DIM.insert code_index (Linear_dynamic_clone {int_index=int_index,code_index=kill_code_index,clone_number=clone_number,dynamic_int=dynamic_int,int=new_int}) new_linear_coroutine),new_size,new_address,new_int_index,new_user_int_index,kill_code_index)
    Case {dynamic_int,int,coroutine_sequence}->do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index,single_linear_coroutine)<-case_coroutine DIM.empty DIS.empty coroutine_sequence int 0 (int_index+clone_number) clone_number (code_index+2) user_int_index int_index address size linear_coroutine
        return (DIM.insert (code_index+1) single_linear_coroutine (DIM.insert code_index (Linear_dynamic_int {int_index=int_index,dynamic_int=dynamic_int}) new_linear_coroutine),new_size,new_address,new_int_index,new_user_int_index,new_code_index)
    Fork {int,coroutine,coroutine_sequence}->let body_code_index=code_index+1 in do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number (body_code_index+int) user_int_index (int_index+clone_number) address size linear_coroutine
        (final_linear_coroutine,first_size,first_address,final_int_index,final_user_int_index,final_code_index)<-fork_coroutine DIS.empty coroutine_sequence body_code_index clone_number (new_code_index+1) new_user_int_index new_int_index new_address new_size (DIM.insert code_index (Linear_int {int_index=int_index,int=int}) new_linear_coroutine)
        return (DIM.insert final_code_index (Linear_kill_fork {int_index=int_index}) final_linear_coroutine,first_size,first_address,final_int_index,final_user_int_index,final_code_index+1)
    Race {dynamic_int,first_int,second_int,coroutine_sequence}->let countdown_int_index=int_index+clone_number in do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index,group_code_index)<-race_coroutine coroutine_sequence 0 countdown_int_index clone_number (code_index+2) user_int_index (int_index+2*clone_number) address size linear_coroutine
        let kill_code_index=new_code_index+3 in return (DIM.insert kill_code_index (Linear_kill_group {int_index=int_index,int=second_int}) (DIM.insert (new_code_index+2) (Linear_yield {code_index=new_code_index}) (DIM.insert (new_code_index+1) (Linear_wake_group {int_index=int_index,dynamic_int=dynamic_int,int=second_int}) (DIM.insert new_code_index (Linear_less_jump {int_index=countdown_int_index,code_index=kill_code_index,int=second_int-first_int+1}) (DIM.insert (code_index+1) (Linear_jump {code_index=new_code_index}) (DIM.insert code_index (Linear_create_group {first_int_index=int_index,second_int_index=countdown_int_index,group_code_index=group_code_index,int=second_int}) new_linear_coroutine))))),new_size,new_address,new_int_index,new_user_int_index,new_code_index+4)

then_coroutine::DS.Seq (Coroutine a)->Int->Int->Int->Int->DVUM.MVector b Int->DVUM.MVector b Int->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVUM.MVector b Int,DVUM.MVector b Int,Int,Int,Int)
then_coroutine coroutine_sequence clone_number code_index user_int_index int_index address size linear_coroutine=case coroutine_sequence of
    DS.Empty->return (linear_coroutine,size,address,int_index,user_int_index,code_index)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index address size linear_coroutine
        then_coroutine other_coroutine clone_number new_code_index new_user_int_index new_int_index new_address new_size new_linear_coroutine

case_coroutine::DIM.IntMap Int->DIS.IntSet->DS.Seq (Coroutine a)->Int->Int->Int->Int->Int->Int->Int->DVUM.MVector b Int->DVUM.MVector b Int->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVUM.MVector b Int,DVUM.MVector b Int,Int,Int,Int,Linear_coroutine a)
case_coroutine case_code_index jump_code_index coroutine_sequence int index max_int_index clone_number code_index user_int_index int_index address size linear_coroutine=case coroutine_sequence of
    DS.Empty->let (new_linear_coroutine,new_code_index)=case_coroutine_a case_code_index int 0 code_index int_index linear_coroutine in return (DIS.foldl' (\this_linear_coroutine this_code_index->DIM.insert this_code_index (Linear_jump {code_index=new_code_index}) this_linear_coroutine) new_linear_coroutine jump_code_index,size,address,max_int_index,user_int_index,new_code_index,Linear_jump {code_index=code_index})
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index (int_index+clone_number) address size linear_coroutine
        case_coroutine (DIM.insert index code_index case_code_index) (DIS.insert new_code_index jump_code_index) other_coroutine int (index+1) (max new_int_index max_int_index) clone_number (new_code_index+1) (max user_int_index new_user_int_index) int_index new_address new_size new_linear_coroutine
...
fork_coroutine::DIS.IntSet->DS.Seq (Coroutine a)->Int->Int->Int->Int->Int->DVUM.MVector b Int->DVUM.MVector b Int->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVUM.MVector b Int,DVUM.MVector b Int,Int,Int,Int)
fork_coroutine jump_code_index coroutine_sequence fork_code_index clone_number code_index user_int_index int_index address size linear_coroutine=case coroutine_sequence of
    DS.Empty->let new_code_index=code_index-1 in return (DIS.foldl' (\this_linear_coroutine this_code_index->DIM.insert this_code_index (Linear_jump {code_index=new_code_index}) this_linear_coroutine) linear_coroutine jump_code_index,size,address,int_index,user_int_index,new_code_index)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index address size linear_coroutine
        fork_coroutine (DIS.insert (code_index-1) jump_code_index) other_coroutine (fork_code_index+1) clone_number (new_code_index+1) new_user_int_index new_int_index new_address new_size (DIM.insert fork_code_index (Linear_fork {code_index=code_index}) new_linear_coroutine)

race_coroutine::DS.Seq (Coroutine a)->Int->Int->Int->Int->Int->Int->DVUM.MVector b Int->DVUM.MVector b Int->DIM.IntMap (Linear_coroutine a)->CMST.ST b (DIM.IntMap (Linear_coroutine a),DVUM.MVector b Int,DVUM.MVector b Int,Int,Int,Int,DIM.IntMap Int)
race_coroutine coroutine_sequence index countdown_int_index clone_number code_index user_int_index int_index address size linear_coroutine=case coroutine_sequence of
    DS.Empty->return (linear_coroutine,size,address,int_index,user_int_index,code_index,DIM.empty)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_size,new_address,new_int_index,new_user_int_index,new_code_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index address size linear_coroutine
        (final_linear_coroutine,final_size,final_address,final_int_index,final_user_int_index,final_code_index,group_code_index)<-race_coroutine other_coroutine (index+1) countdown_int_index clone_number (new_code_index+1) new_user_int_index (max int_index new_int_index) new_address new_size (DIM.insert new_code_index (Linear_countdown {int_index=countdown_int_index}) new_linear_coroutine)
        return (final_linear_coroutine,final_size,final_address,final_int_index,final_user_int_index,final_code_index,DIM.insert index code_index group_code_index)

step_coroutine::DV.Vector (Linear_coroutine a)->Int->Int->DS.Seq Int->DS.Seq Int->DS.Seq Int->DIM.IntMap (DS.Seq Int)->DIM.IntMap Program_counter->DVU.Vector Int->DVU.Vector Int->DVU.Vector Int->DVUM.MVector b Int->((Engine a->Engine a)->Engine a->Engine a)->DS.Seq (Engine a->Engine a)->Event->Engine a->Widget a->CMST.ST b (Widget a,Engine a,DS.Seq (Engine a->Engine a),DVUM.MVector b Int,DVU.Vector Int,DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int),DS.Seq Int,Int,Int)
step_coroutine linear_coroutine program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget=case newborn_main_index_group of
    DS.Empty->case main_index_group of
        ...
        (main_index DS.:<| other_main_index)->let (new_program_counter,single_program_counter)=intmap_delete_lookup main_index program_counter in step_coroutine_a linear_coroutine main_index single_program_counter.clone_index single_program_counter.code_index program_counter_index index_group_index survived_main_index_group DS.empty other_main_index index_group new_program_counter address size user_variable variable updater update event engine widget
    (main_index DS.:<| other_main_index)->let (new_program_counter,single_program_counter)=intmap_delete_lookup main_index program_counter in step_coroutine_a linear_coroutine main_index single_program_counter.clone_index single_program_counter.code_index program_counter_index index_group_index survived_main_index_group DS.empty (other_main_index DS.>< main_index_group) index_group new_program_counter address size user_variable variable updater update event engine widget

step_coroutine_a::DV.Vector (Linear_coroutine a)->Int->Int->Int->Int->Int->DS.Seq Int->DS.Seq Int->DS.Seq Int->DIM.IntMap (DS.Seq Int)->DIM.IntMap Program_counter->DVU.Vector Int->DVU.Vector Int->DVU.Vector Int->DVUM.MVector b Int->((Engine a->Engine a)->Engine a->Engine a)->DS.Seq (Engine a->Engine a)->Event->Engine a->Widget a->CMST.ST b (Widget a,Engine a,DS.Seq (Engine a->Engine a),DVUM.MVector b Int,DVU.Vector Int,DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int),DS.Seq Int,Int,Int)
step_coroutine_a linear_coroutine main_index clone_index this_code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget=case linear_coroutine DV.! this_code_index of
    Linear_end->step_coroutine linear_coroutine program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
    Linear_emit {emit}->let (new_widget,new_update)=emit event engine widget in step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater (update DS.|> new_update) event (updater new_update engine) new_widget
    Linear_wait {int_index}->do
        int<-DVUM.read variable (int_index+clone_index)
        DVUM.write variable (int_index+clone_index) (int-1)
        if 0<int then step_coroutine linear_coroutine program_counter_index index_group_index (survived_main_index_group DS.|> main_index) newborn_main_index_group main_index_group index_group (intmap_insert main_index (Program_counter {code_index=this_code_index,clone_index=clone_index}) program_counter) address size user_variable variable updater update event engine widget else step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
    ...
    Linear_countdown {int_index}->do
        ...
        step_coroutine linear_coroutine program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
    ...
    Linear_kill_clone {int_index,clone_number}->let new_clone_index=mod clone_index clone_number in do
        ...
        if 0<int then step_coroutine linear_coroutine program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget else step_coroutine_a linear_coroutine main_index new_clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
    Linear_dynamic_int {int_index,dynamic_int}->do
        DVUM.write variable (int_index+clone_index) (dynamic_int.dynamic_int (user_variable_getter user_variable address size clone_index) event engine widget)
        step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
    ...
    Linear_kill_group {int_index,int}->do
        ...
        let (new_program_counter,new_index_group)=run_kill_group int 0 this_int index_group program_counter in step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group new_index_group new_program_counter address size user_variable variable updater update event engine widget
    Linear_true_jump {code_index,dynamic_bool}->if dynamic_bool.dynamic_bool (user_variable_getter user_variable address size clone_index) event engine widget then step_coroutine_a linear_coroutine main_index clone_index code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget else step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
    Linear_false_jump {code_index,dynamic_bool}->if dynamic_bool.dynamic_bool (user_variable_getter user_variable address size clone_index) event engine widget then step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget else step_coroutine_a linear_coroutine main_index clone_index code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
    ...
    Linear_clone {int_index,clone_number,int}->do
        DVUM.write variable (int_index+clone_index) int
        let new_code_index=this_code_index+1 in let (new_program_counter,new_newborn_main_index_group,new_program_counter_index)=run_clone int clone_number (clone_index+clone_number) new_code_index program_counter_index newborn_main_index_group program_counter in step_coroutine_a linear_coroutine main_index clone_index new_code_index new_program_counter_index index_group_index survived_main_index_group new_newborn_main_index_group main_index_group index_group new_program_counter address size user_variable variable updater update event engine widget
    Linear_wake_group {int_index,dynamic_int,int}->let new_int=dynamic_int.dynamic_int (user_variable_getter user_variable address size clone_index) event engine widget in if new_int<0||int<=new_int then EE.quick_error "step_coroutine_a" 0 else do
        this_int<-DVUM.read variable (int_index+clone_index)
        let new_index_group_index=this_int+new_int in let (new_index_group_map,maybe_single_index_group)=intmap_delete_maybe_lookup new_index_group_index index_group in case maybe_single_index_group of
            Nothing->step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget
            Just single_index_group->do
                (new_widget,new_engine,new_update,new_variable,new_user_variable,new_program_counter,new_index_group,new_survived_main_index_group,new_new_index_group_index,new_program_counter_index)<-step_coroutine linear_coroutine program_counter_index index_group_index DS.empty DS.empty single_index_group new_index_group_map program_counter address size user_variable variable updater DS.empty event engine widget
                step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) new_program_counter_index new_new_index_group_index survived_main_index_group newborn_main_index_group main_index_group (if DS.null new_survived_main_index_group then new_index_group else DIM.insert new_index_group_index new_survived_main_index_group new_index_group) new_program_counter address size new_user_variable new_variable updater (update DS.>< new_update) event new_engine new_widget
    Linear_assign {user_int_index,clone_number,dynamic_int}->do
        new_user_variable<-DVU.unsafeThaw user_variable
        DVUM.write new_user_variable (user_int_index+mod clone_index clone_number) (dynamic_int.dynamic_int (user_variable_getter user_variable address size clone_index) event engine widget)
        new_new_user_variable<-DVU.unsafeFreeze new_user_variable
        step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size new_new_user_variable variable updater update event engine widget
    ...
    Linear_dynamic_clone {int_index,code_index,clone_number,dynamic_int,int}->let new_int=dynamic_int.dynamic_int (user_variable_getter user_variable address size clone_index) event engine widget-1 in if new_int<0 then step_coroutine_a linear_coroutine main_index clone_index code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter address size user_variable variable updater update event engine widget else if int<new_int then EE.quick_error "step_coroutine_a" 1 else let new_code_index=this_code_index+1 in let (new_program_counter,new_newborn_main_index_group,new_program_counter_index)=run_clone new_int clone_number (clone_index+clone_number) new_code_index program_counter_index newborn_main_index_group program_counter in do
        DVUM.write variable (int_index+clone_index) new_int
        step_coroutine_a linear_coroutine main_index clone_index new_code_index new_program_counter_index index_group_index survived_main_index_group new_newborn_main_index_group main_index_group index_group new_program_counter address size user_variable variable updater update event engine widget

user_variable_getter::DVU.Vector Int->DVU.Vector Int->DVU.Vector Int->Int->Int->Int
user_variable_getter user_variable address size clone_index int=user_variable DVU.! (address DVU.! int+mod clone_index (size DVU.! int))
