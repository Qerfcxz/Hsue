...
from_coroutine coroutine int=CMST.runST $ do
    ...
    (linear_coroutine,new_layout,int_index,user_int_index,code_index,_)<-from_coroutine_a coroutine 1 0 0 0 layout DIM.empty
...
from_coroutine_a::Coroutine a b c d e->Int->Int->Int->Int->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int,DIS.IntSet)
from_coroutine_a this_coroutine clone_number code_index user_int_index int_index layout linear_coroutine=case this_coroutine of
    Done->return (linear_coroutine,layout,int_index,user_int_index,code_index,DIS.empty)
    Emit {emit}->return (DIM.insert code_index (Linear_emit {emit=emit}) linear_coroutine,layout,int_index,user_int_index,code_index+1,DIS.empty)
    Wait {dynamic_int}->return (DIM.insert (code_index+1) (Linear_wait {int_index=int_index}) (DIM.insert code_index (Linear_dynamic_int {int_index=int_index,dynamic_int=dynamic_int}) linear_coroutine),layout,int_index+clone_number,user_int_index,code_index+2,DIS.empty)
    Forever {coroutine}->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index layout linear_coroutine
    ...
    While {dynamic_bool,coroutine}->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine clone_number (code_index+1) user_int_index int_index layout linear_coroutine
        ...
        case single_layout of
            Layout {address,size}->if size==0
                then do
                    ...
                    return (DIM.insert code_index (Linear_assign {user_int_index=user_int_index,clone_number=clone_number,dynamic_int=dynamic_int}) linear_coroutine,layout,int_index,user_int_index+clone_number,code_index+1,DIS.empty)
                else return (DIM.insert code_index (Linear_assign {user_int_index=address,clone_number=size,dynamic_int=dynamic_int}) linear_coroutine,layout,int_index,user_int_index,code_index+1,DIS.empty)
    Repeat {dynamic_int,coroutine}->let loop_code_index=code_index+2 in do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine clone_number loop_code_index user_int_index (int_index+clone_number) layout linear_coroutine
    ...
    Clone {int,coroutine}->let new_int=int-1 in if new_int<0 then return (linear_coroutine,layout,int_index,user_int_index,code_index,DIS.empty) else do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine (int*clone_number) (code_index+1) user_int_index (int_index+clone_number) layout linear_coroutine
    ...
    If {dynamic_bool,first_coroutine,second_coroutine}->do
        (first_linear_coroutine,first_layout,first_int_index,first_user_int_index,first_code_index,first_group_int_index)<-from_coroutine_a first_coroutine clone_number (code_index+1) user_int_index int_index layout linear_coroutine
        let new_code_index=first_code_index+1
        (second_linear_coroutine,second_layout,second_int_index,second_user_int_index,second_code_index,second_group_int_index)<-from_coroutine_a second_coroutine clone_number new_code_index first_user_int_index int_index first_layout first_linear_coroutine
        return (DIM.insert first_code_index (Linear_jump {code_index=second_code_index}) (DIM.insert code_index (Linear_false_jump {code_index=new_code_index,dynamic_bool=dynamic_bool}) second_linear_coroutine),second_layout,max first_int_index second_int_index,max first_user_int_index second_user_int_index,second_code_index,DIS.union first_group_int_index second_group_int_index)
    Dynamic_clone {dynamic_int,int,coroutine}->let new_int=int-1 in if new_int<0 then return (linear_coroutine,layout,int_index,user_int_index,code_index,DIS.empty) else do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine (int*clone_number) (code_index+1) user_int_index (int_index+clone_number) layout linear_coroutine
        let kill_code_index=new_code_index+1 in return (DIM.insert new_code_index (Linear_kill_clone {int_index=int_index,clone_number=clone_number}) (DIM.insert code_index (Linear_dynamic_clone {int_index=int_index,code_index=kill_code_index,clone_number=clone_number,dynamic_int=dynamic_int,int=new_int}) new_linear_coroutine),new_layout,new_int_index,new_user_int_index,kill_code_index,clone_coroutine group_int_index int clone_number)
    Case {dynamic_int,int,coroutine_sequence}->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,single_linear_coroutine,group_int_index)<-case_coroutine DIM.empty DIS.empty coroutine_sequence int 0 (int_index+clone_number) clone_number (code_index+2) user_int_index int_index layout linear_coroutine
    ...
    Fork {int,coroutine,coroutine_sequence}->let countdown_int_index=int_index+clone_number in do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,main_group_int_index)<-from_coroutine_a coroutine clone_number (code_index+2) user_int_index (int_index+2*clone_number) layout linear_coroutine
        let loop_code_index=new_code_index
        let kill_code_index=loop_code_index+2
        let jump_code_index=kill_code_index+1
        (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,fork_end_code_index,group_code_index,fork_group_int_index)<-let fork_start_code_index=jump_code_index+1 in fork_coroutine coroutine_sequence 0 countdown_int_index clone_number fork_start_code_index new_user_int_index new_int_index new_layout new_linear_coroutine
        let all_group_int_index=DIS.insert int_index (DIS.union main_group_int_index fork_group_int_index) in return (DIM.insert jump_code_index (Linear_jump {code_index=fork_end_code_index}) (DIM.insert kill_code_index (Linear_kill_group {group_int_index=all_group_int_index,int=int}) (DIM.insert (loop_code_index+1) (Linear_yield {code_index=loop_code_index}) (DIM.insert loop_code_index (Linear_less_jump {int_index=countdown_int_index,code_index=kill_code_index,int=1}) (DIM.insert (code_index+1) (Linear_jump {code_index=code_index+2}) (DIM.insert code_index (Linear_create_active_group {first_int_index=int_index,second_int_index=countdown_int_index,group_code_index=group_code_index,int=int}) final_linear_coroutine))))),final_layout,final_int_index,final_user_int_index,fork_end_code_index,all_group_int_index)
    Race {dynamic_int,first_int,second_int,coroutine_sequence}->let countdown_int_index=int_index+clone_number in do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_code_index,group_int_index)<-race_coroutine coroutine_sequence 0 countdown_int_index clone_number (code_index+2) user_int_index (int_index+2*clone_number) layout linear_coroutine
        let kill_code_index=new_code_index+3 in return (DIM.insert kill_code_index (Linear_kill_group {group_int_index=DIS.insert int_index group_int_index,int=second_int}) (DIM.insert (new_code_index+2) (Linear_yield {code_index=new_code_index}) (DIM.insert (new_code_index+1) (Linear_wake_group {int_index=int_index,dynamic_int=dynamic_int,int=second_int}) (DIM.insert new_code_index (Linear_less_jump {int_index=countdown_int_index,code_index=kill_code_index,int=second_int-first_int+1}) (DIM.insert (code_index+1) (Linear_jump {code_index=new_code_index}) (DIM.insert code_index (Linear_create_group {first_int_index=int_index,second_int_index=countdown_int_index,group_code_index=group_code_index,int=second_int}) new_linear_coroutine))))),new_layout,new_int_index,new_user_int_index,new_code_index+4,DIS.insert int_index group_int_index)

then_coroutine::DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int,DIS.IntSet)
then_coroutine coroutine_sequence clone_number code_index user_int_index int_index layout linear_coroutine=case coroutine_sequence of
    DS.Empty->return (linear_coroutine,layout,int_index,user_int_index,code_index,DIS.empty)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index layout linear_coroutine
        (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,final_group_int_index)<-then_coroutine other_coroutine clone_number new_code_index new_user_int_index new_int_index new_layout new_linear_coroutine
        return (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,DIS.union group_int_index final_group_int_index)

clone_coroutine::DIS.IntSet->Int->Int->DIS.IntSet
clone_coroutine group_int_index int clone_number=DIS.foldl' (\this_group_int_index single_int_index->clone_coroutine_a int 0 single_int_index clone_number this_group_int_index) DIS.empty group_int_index

clone_coroutine_a::Int->Int->Int->Int->DIS.IntSet->DIS.IntSet
clone_coroutine_a int index single_int_index clone_number intset=if int<=index then intset else clone_coroutine_a int (index+1) single_int_index clone_number (DIS.insert (single_int_index+index*clone_number) intset)

case_coroutine::DIM.IntMap Int->DIS.IntSet->DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->Int->Int->Int->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int,Linear_coroutine a b c d e,DIS.IntSet)
case_coroutine case_code_index jump_code_index coroutine_sequence int index max_int_index clone_number code_index user_int_index int_index layout linear_coroutine=case coroutine_sequence of
    DS.Empty->let (new_linear_coroutine,new_code_index)=case_coroutine_a case_code_index int 0 code_index int_index linear_coroutine in return (DIS.foldl' (\this_linear_coroutine this_code_index->DIM.insert this_code_index (Linear_jump {code_index=new_code_index}) this_linear_coroutine) new_linear_coroutine jump_code_index,layout,max_int_index,user_int_index,new_code_index,Linear_jump {code_index=code_index},DIS.empty)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine clone_number code_index user_int_index (int_index+clone_number) layout linear_coroutine
        ...
        return (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,single_linear_coroutine,DIS.union group_int_index final_group_int_index)
...
fork_coroutine::DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->Int->Int->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int,DIM.IntMap Int,DIS.IntSet)
fork_coroutine coroutine_sequence index countdown_int_index clone_number code_index user_int_index int_index layout linear_coroutine=case coroutine_sequence of
    DS.Empty->return (linear_coroutine,layout,int_index,user_int_index,code_index,DIM.empty,DIS.empty)
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index layout linear_coroutine
        (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,group_code_index,final_group_int_index)<-fork_coroutine other_coroutine (index+1) countdown_int_index clone_number (new_code_index+1) new_user_int_index (max int_index new_int_index) new_layout (DIM.insert new_code_index (Linear_countdown {int_index=countdown_int_index}) new_linear_coroutine)
        return (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,DIM.insert index code_index group_code_index,DIS.union group_int_index final_group_int_index)

race_coroutine::DS.Seq (Coroutine a b c d e)->Int->Int->Int->Int->Int->Int->DVSM.MVector f Layout->DIM.IntMap (Linear_coroutine a b c d e)->CMST.ST f (DIM.IntMap (Linear_coroutine a b c d e),DVSM.MVector f Layout,Int,Int,Int,DIM.IntMap Int,DIS.IntSet)
race_coroutine coroutine_sequence index countdown_int_index clone_number code_index user_int_index int_index layout linear_coroutine=case coroutine_sequence of
    ...
    coroutine DS.:<| other_coroutine->do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine clone_number code_index user_int_index int_index layout linear_coroutine
        (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,group_code_index,final_group_int_index)<-race_coroutine other_coroutine (index+1) countdown_int_index clone_number (new_code_index+1) new_user_int_index (max int_index new_int_index) new_layout (DIM.insert new_code_index (Linear_countdown {int_index=countdown_int_index}) new_linear_coroutine)
        return (final_linear_coroutine,final_layout,final_int_index,final_user_int_index,final_code_index,DIM.insert index code_index group_code_index,DIS.union group_int_index final_group_int_index)
...
step_coroutine_a linear_coroutine main_index clone_index this_code_index program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget=case linear_coroutine DV.! this_code_index of
    ...
    Linear_int {int_index,int}->do
        ...
        step_coroutine_a linear_coroutine main_index clone_index (this_code_index+1) program_counter_index index_group_index survived_main_index_group newborn_main_index_group main_index_group index_group program_counter layout user_variable variable updater update event engine widget
    Linear_kill_group {group_int_index,int}->do
        (new_program_counter,new_index_group)<-intset_monad_fold (\int_index (this_program_counter,this_index_group)->run_kill_group int int_index clone_index variable this_program_counter this_index_group) group_int_index (program_counter,index_group)
...
run_kill_group::Int->Int->Int->DVUM.MVector f Int->DIM.IntMap Program_counter->DIM.IntMap (DS.Seq Int)->CMST.ST f (DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int))
run_kill_group int int_index clone_index variable program_counter index_group=do
    this_int<-DVUM.read variable (int_index+clone_index)
    if this_int<=0 then return (program_counter,index_group) else do
        DVUM.write variable (int_index+clone_index) (-1)
        return (run_kill_group_a int 0 this_int index_group program_counter)
...
run_create_active_group::DIM.IntMap Int->Int->Int->Int->Int->Int->DIM.IntMap (DS.Seq Int)->DIM.IntMap Program_counter->DS.Seq Int->(DIM.IntMap Program_counter,DIM.IntMap (DS.Seq Int),Int,DS.Seq Int)
run_create_active_group group_code_index int index clone_index program_counter_index index_group_index index_group program_counter newborn_main_index_group=if int<=index then (program_counter,index_group,program_counter_index,newborn_main_index_group) else run_create_active_group group_code_index int (index+1) clone_index (program_counter_index+1) index_group_index (intmap_insert (index_group_index+index) (DS.singleton program_counter_index) index_group) (intmap_insert program_counter_index (Program_counter {code_index=intmap_lookup index group_code_index,clone_index=clone_index}) program_counter) (newborn_main_index_group DS.|> program_counter_index)