...
run_coroutine_b::Event a->Engine b a c d e->Bool->DV.Vector (Linear_coroutine b a c d e)->DVS.Vector Layout->Coroutine_state b a c d e->DS.Seq (Engine b a c d e->Engine b a c d e)->(DS.Seq (Engine b a c d e->Engine b a c d e),Coroutine_state b a c d e)
...
from_coroutine_a this_coroutine layout_size clone_number code_index user_int_index int_index layout linear_coroutine=case this_coroutine of
    ...
    Clone {int,coroutine}->if int<=0 then EE.empty_error else do
    ...
    Dynamic_clone {dynamic_int,int,coroutine}->if int<=0 then EE.empty_error else do
        (new_linear_coroutine,new_layout,new_int_index,new_user_int_index,new_code_index,group_int_index)<-from_coroutine_a coroutine layout_size (int*clone_number) (code_index+1) user_int_index (int_index+clone_number) layout linear_coroutine
    ...
    Fork {int,coroutine,coroutine_sequence}->if int<=0 then EE.empty_error else do
