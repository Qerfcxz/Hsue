...
to_collect u v widget=case widget of
    Visual {origin,matrix,clip,red,green,blue,alpha,visual}->let parameter=Parameter {x=matrix.x,y=matrix.y,x_x=matrix.x_x,x_y=matrix.x_y,y_x=matrix.y_x,y_y=matrix.y_y,clip_left=origin.x+clip.left,clip_down=origin.y+clip.down,clip_right=origin.x+clip.right,clip_up=origin.y+clip.up} in case visual of
    ...
    Text {origin,matrix,width,height,y,text}->let new_width=width/2 in let new_height=height/2 in let (vertex,index,_)=DF.foldl' (DF.foldl' (flip (collect_text origin y))) (DS.empty,DS.empty,0) (fmap (DS.takeWhileL (end_text y new_height) . DS.dropWhileL (begin_text y new_height)) text) in Submit {maybe_album_id=Nothing,vertex=vertex,index=index,parameter=Parameter {x=matrix.x,y=matrix.y,x_x=matrix.x_x,x_y=matrix.x_y,y_x=matrix.y_x,y_y=matrix.y_y,clip_left=origin.x-new_width,clip_down=origin.y-new_height,clip_right=origin.x+new_width,clip_up=origin.y+new_height},vertex_length=fromIntegral (DS.length vertex),index_length=fromIntegral (DS.length index)}
...
collect_text origin this_y row primitive=case row of
    Blank->primitive
    Row {seq_character,x,y}->let new_y=this_y-y in DF.foldl' (flip (collect_character origin x new_y)) primitive seq_character
