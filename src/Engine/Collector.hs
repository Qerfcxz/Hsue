...
to_collect u v widget=case widget of
    Visual {origin,matrix,red,green,blue,alpha,visual}->case visual of
        Triangle {first_point,second_point,third_point}->let new_first_point=apply_matrix matrix (point_addition origin first_point) in let new_second_point=apply_matrix matrix (point_addition origin second_point) in let new_third_point=apply_matrix matrix (point_addition origin third_point) in Submit {maybe_album_id=Nothing,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=new_first_point.x,y=new_first_point.y,u=u,v=v}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=new_second_point.x,y=new_second_point.y,u=u,v=v} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=new_third_point.x,y=new_third_point.y,u=u,v=v},index=DS.singleton 0 DS.|> 1 DS.|> 2,vertex_length=3,index_length=3}
        Convex_polygon {point}->let vertex=fmap ((\this_point->Vertex {red=red,green=green,blue=blue,alpha=alpha,x=this_point.x,y=this_point.y,u=u,v=v}) . apply_matrix matrix . point_addition origin) point in let number=DS.length point in if number<3 then error "to_collect: error 1" else let new_number=3*(number-2) in Submit {maybe_album_id=Nothing,vertex=vertex,index=DS.fromFunction new_number for_convex_polygon,vertex_length=fromIntegral number,index_length=fromIntegral new_number}
        Regular_polygon {number,radius,angle}->if number<3 then error "to_collect: error 2" else let new_number=3*(number-2) in let new_angle=2*pi/fromIntegral number in Submit {maybe_album_id=Nothing,vertex=fmap ((\point->Vertex {red=red,green=green,blue=blue,alpha=alpha,x=point.x,y=point.y,u=u,v=v}) . apply_matrix matrix) (DS.fromFunction number (\index->let direction=angle+fromIntegral index*new_angle in Point {x=origin.x+radius*cos direction,y=origin.y+radius*sin direction})),index=DS.fromFunction (3*(number-2)) for_convex_polygon,vertex_length=fromIntegral number,index_length=fromIntegral new_number}
        Picture {width,height,min_u,min_v,max_u,max_v}->let new_width=width/2 in let new_height=height/2 in let first_point=apply_matrix matrix (Point {x=origin.x-new_width,y=origin.y-new_height}) in let second_point=apply_matrix matrix (Point {x=origin.x+new_width,y=origin.y-new_height}) in let third_point=apply_matrix matrix (Point {x=origin.x+new_width,y=origin.y+new_height}) in let fourth_point=apply_matrix matrix (Point {x=origin.x-new_width,y=origin.y+new_height}) in Submit {maybe_album_id=Nothing,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=first_point.x,y=first_point.y,u=min_u,v=min_v}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=second_point.x,y=second_point.y,u=max_u,v=min_v} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=third_point.x,y=third_point.y,u=max_u,v=max_v} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=fourth_point.x,y=fourth_point.y,u=min_u,v=max_v},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,vertex_length=4,index_length=6}
        Large_picture {width,height,album_id}->let new_width=width/2 in let new_height=height/2 in let first_point=apply_matrix matrix (Point {x=origin.x-new_width,y=origin.y-new_height}) in let second_point=apply_matrix matrix (Point {x=origin.x+new_width,y=origin.y-new_height}) in let third_point=apply_matrix matrix (Point {x=origin.x+new_width,y=origin.y+new_height}) in let fourth_point=apply_matrix matrix (Point {x=origin.x-new_width,y=origin.y+new_height}) in Submit {maybe_album_id=Just album_id,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=first_point.x,y=first_point.y,u=0,v=1}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=second_point.x,y=second_point.y,u=1,v=1} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=third_point.x,y=third_point.y,u=1,v=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=fourth_point.x,y=fourth_point.y,u=0,v=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,vertex_length=4,index_length=6}
