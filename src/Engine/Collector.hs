    ...
    Visual {origin,matrix,red,green,blue,alpha,visual}->let parameter=to_Parameter origin matrix maybe_border in case visual of
        ...
        Atlas {clip,index,locked}->if locked then EE.quick_error "to_collect" 3 else case origin of
            Point {x=origin_x,y=origin_y}->case clip DVS.! index of
                Clip {x,y,width,height,min_u,min_v,max_u,max_v}->let new_width=width/2 in let new_height=height/2 in let new_x=origin_x+x in let new_y=origin_y+y in let left=new_x-new_width in let down=new_y-new_height in let right=new_x+new_width in let up=new_y+new_height in Submit {maybe_canvas_id=Nothing,maybe_album_id=Nothing,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=min_u,v=max_v,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=max_u,v=max_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=max_u,v=min_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=min_u,v=min_v,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
        Large_atlas {clip,album_id,index}->case origin of
            Point {x=origin_x,y=origin_y}->case clip DVS.! index of
                Clip {x,y,width,height,min_u,min_v,max_u,max_v}->let new_width=width/2 in let new_height=height/2 in let new_x=origin_x+x in let new_y=origin_y+y in let left=new_x-new_width in let down=new_y-new_height in let right=new_x+new_width in let up=new_y+new_height in Submit {maybe_canvas_id=Nothing,maybe_album_id=Just album_id,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=min_u,v=max_v,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=max_u,v=max_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=max_u,v=min_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=min_u,v=min_v,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
        Animation {frame_width,frame_height,width,height,padding,width_number,height_number,album_number,album_id,index}->case origin of
            Point {x,y}->let new_frame_width=frame_width/2 in let new_frame_height=frame_height/2 in let left=x-new_frame_width in let down=y-new_frame_height in let right=x+new_frame_width in let up=y+new_frame_height in let (quotient,remainder)=divMod index (width_number*height_number) in let new_album_id=album_id+quotient in if album_number<=quotient then EE.quick_error "to_collect" 4 else let (new_quotient,new_remainder)=divMod remainder width_number in let frame_x=fromIntegral new_remainder*(frame_width+2*padding)+padding in let frame_y=fromIntegral new_quotient*(frame_height+2*padding)+padding in let min_u=frame_x/width in let min_v=frame_y/height in let max_u=(frame_x+frame_width)/width in let max_v=(frame_y+frame_height)/height in Submit {maybe_canvas_id=Nothing,maybe_album_id=Just new_album_id,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=min_u,v=max_v,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=max_u,v=max_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=max_u,v=min_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=min_u,v=min_v,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
        Canvas {canvas_width,canvas_height,canvas_id,locked}->if locked then EE.quick_error "to_collect" 5 else case origin of
            Point {x,y}->let new_width=fromIntegral canvas_width/2 in let new_height=fromIntegral canvas_height/2 in let left=x-new_width in let down=y-new_height in let right=x+new_width in let up=y+new_height in Submit {maybe_canvas_id=Just canvas_id,maybe_album_id=Nothing,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=0,v=1,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=1,v=1,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=1,v=0,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=0,v=0,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
    Text {origin,matrix,width,height,y,article}->case origin of
        Point {x=origin_x,y=origin_y}->case maybe_border of
            Nothing->let new_width=width/2 in let new_height=height/2 in let (vertex,index,_)=DF.foldl' (DF.foldl' (flip (collect_text origin_x origin_y y))) (DS.empty,DS.empty,0) (fmap (DS.takeWhileL (end_text y new_height) . DS.dropWhileL (begin_text y new_height)) article) in Submit {maybe_canvas_id=Nothing,maybe_album_id=Nothing,vertex=vertex,index=index,parameter=Parameter {x=origin_x+matrix.x,y=origin_y+matrix.y,x_x=matrix.x_x,x_y=matrix.x_y,y_x=matrix.y_x,y_y=matrix.y_y,border_flag=1,border_left=origin_x-new_width,border_down=origin_y-new_height,border_right=origin_x+new_width,border_up=origin_y+new_height},vertex_length=fromIntegral (DS.length vertex),index_length=fromIntegral (DS.length index)}
            Just border->case border of
                Border {left,down,right,up}->let new_width=width/2 in let new_height=height/2 in let border_down=max (-new_height) down in let border_up=min new_height up in let (vertex,index,_)=DF.foldl' (DF.foldl' (flip (collect_text origin_x origin_y y))) (DS.empty,DS.empty,0) (fmap (DS.takeWhileL (end_text y (-border_down)) . DS.dropWhileL (begin_text y border_up)) article) in Submit {maybe_canvas_id=Nothing,maybe_album_id=Nothing,vertex=vertex,index=index,parameter=Parameter {x=origin_x+matrix.x,y=origin_y+matrix.y,x_x=matrix.x_x,x_y=matrix.x_y,y_x=matrix.y_x,y_y=matrix.y_y,border_flag=1,border_left=origin_x+max (-new_width) left,border_down=origin_y+border_down,border_right=origin_x+min new_width right,border_up=origin_y+border_up},vertex_length=fromIntegral (DS.length vertex),index_length=fromIntegral (DS.length index)}
