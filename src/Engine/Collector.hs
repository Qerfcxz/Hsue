...
to_collect_visual::ET.Has_call_stack=>Maybe Arrange->FCT.CFloat->FCT.CFloat->Maybe (Border FCT.CFloat)->Visual->Submit
to_collect_visual maybe_arrange u v maybe_border visual=case visual of
    ...
    Picture {arrange,half_width,half_height,min_u,min_v,max_u,max_v,locked}->if locked then EF.empty_error else case maybe arrange (`combine_arrange` arrange) maybe_arrange of
        Arrange {point,matrix,color}->case color of
            Color {red,green,blue,alpha}->case point of
    ...
    Atlas {arrange,clip,index,locked}->if locked then EF.empty_error else case maybe arrange (`combine_arrange` arrange) maybe_arrange of
        Arrange {point,matrix,color}->case color of
    ...
    Text {arrange,half_width,half_height,current_y,article,locked}->if locked then EF.empty_error else case maybe arrange (`combine_arrange` arrange) maybe_arrange of
        Arrange {point,matrix,color}->case color of
            Color {red,green,blue,alpha}->case point of
    ...
    Canvas {arrange,half_width,half_height,canvas_id}->case maybe arrange (`combine_arrange` arrange) maybe_arrange of
        Arrange {point,matrix,color}->case color of
            Color {red,green,blue,alpha}->case point of
                Point {x,y}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in Submit {submit_mode=Submit_canvas {canvas_id=canvas_id},vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=0,v=1,parameter_id=0,font_size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=1,v=1,parameter_id=0,font_size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=1,v=0,parameter_id=0,font_size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=0,v=0,parameter_id=0,font_size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter x y matrix maybe_border,vertex_size=4,index_size=6}
...
collect_text::ET.Has_call_stack=>FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->Row->(DS.Seq Vertex,DS.Seq DW.Word32,DW.Word32)->(DS.Seq Vertex,DS.Seq DW.Word32,DW.Word32)
