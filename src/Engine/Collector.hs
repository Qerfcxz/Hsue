...
maybe_update_collect update view maybe_border projection_path leaf_id selector collect_strategy engine=let maybe_tuple=DFC.getCompose (functor_lookup_projection_widget projection_path (\widget->DFC.Compose {getCompose=fmap (\this_widget->(to_collect engine.u engine.v maybe_border (view this_widget),this_widget)) (selector_monad_update (const update) selector widget)}) engine) in case maybe_tuple of
    Nothing->engine
...
to_collect u v maybe_border widget=case lookup_widget widget of
    Visual {origin,matrix,red,green,blue,alpha,visual}->let parameter=to_Parameter origin matrix maybe_border in case visual of
        ...
        Animation {frame_width,frame_height,width,height,padding,width_number,height_number,album_number,album_id,index}->case origin of
            Point {x,y}->let new_frame_width=frame_width/2 in let new_frame_height=frame_height/2 in let left=x-new_frame_width in let down=y-new_frame_height in let right=x+new_frame_width in let up=y+new_frame_height in let (quotient,remainder)=divMod index (width_number*height_number) in let new_album_id=album_id+quotient in if album_number<=quotient then EE.quick_error "to_collect" 4 else let (new_quotient,new_remainder)=divMod remainder width_number in let row=fromIntegral new_quotient in let column=fromIntegral new_remainder in let pack_width=frame_width+2*padding in let pack_height=frame_height+2*padding in let frame_x=column*pack_width+padding in let frame_y=row*pack_height+padding in let min_u=frame_x/width in let min_v=frame_y/height in let max_u=(frame_x+frame_width)/width in let max_v=(frame_y+frame_height)/height in Submit {maybe_album_id=Just new_album_id,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=min_u,v=max_v,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=max_u,v=max_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=max_u,v=min_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=min_u,v=min_v,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
...
move::Projection_move->Int->Insert_strategy->Engine a b c d e->Engine a b c d e
