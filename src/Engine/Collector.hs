...
collect_canvas::ET.Has_call_stack=>Point->Matrix->FCT.CFloat->FCT.CFloat->FCT.CFloat->FCT.CFloat->Maybe (Border FCT.CFloat)->Int->Int->Selector a->Insert_strategy->Engine b c d e f->Engine b c d e f
...
to_collect_visual arrange_transform u v maybe_border visual=case visual of
    Rectangle {arrange,half_width,half_height}->case arrange_transform arrange of
        Arrange {point,matrix,color}->case point of
            Point {x,y}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in Submit {submit_mode=Submit_default,vertex=DS.singleton (quick_create_vertex color left down u v) DS.|> quick_create_vertex color right down u v DS.|> quick_create_vertex color right up u v DS.|> quick_create_vertex color left up u v,index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter x y matrix maybe_border,vertex_size=4,index_size=6}
    ...
    Picture {arrange,half_width,half_height,min_u,min_v,max_u,max_v,locked}->if locked then EF.empty_error else case arrange_transform arrange of
        Arrange {point,matrix,color}->case point of
            Point {x,y}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in Submit {submit_mode=Submit_default,vertex=DS.singleton (quick_create_vertex color left down min_u max_v) DS.|> quick_create_vertex color right down max_u max_v DS.|> quick_create_vertex color right up max_u min_v DS.|> quick_create_vertex color left up min_u min_v,index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter x y matrix maybe_border,vertex_size=4,index_size=6}
    Large_picture {arrange,half_width,half_height,album_id}->case arrange_transform arrange of
        Arrange {point,matrix,color}->case point of
            Point {x,y}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in Submit {submit_mode=Submit_album {album_id=album_id},vertex=DS.singleton (quick_create_vertex color left down 0 1) DS.|> quick_create_vertex color right down 1 1 DS.|> quick_create_vertex color right up 1 0 DS.|> quick_create_vertex color left up 0 0,index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter x y matrix maybe_border,vertex_size=4,index_size=6}
    Atlas {arrange,clip,index,locked}->if locked then EF.empty_error else case arrange_transform arrange of
        Arrange {point,matrix,color}->case move_clip point (clip DVS.! index) of
            Clip {x,y,half_width,half_height,min_u,min_v,max_u,max_v}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in Submit {submit_mode=Submit_default,vertex=DS.singleton (quick_create_vertex color left down min_u max_v) DS.|> quick_create_vertex color right down max_u max_v DS.|> quick_create_vertex color right up max_u min_v DS.|> quick_create_vertex color left up min_u min_v,index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter point.x point.y matrix maybe_border,vertex_size=4,index_size=6}
    Large_atlas {arrange,clip,album_id,index}->case arrange_transform arrange of
        Arrange {point,matrix,color}->case move_clip point (clip DVS.! index) of
            Clip {x,y,half_width,half_height,min_u,min_v,max_u,max_v}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in Submit {submit_mode=Submit_album {album_id=album_id},vertex=DS.singleton (quick_create_vertex color left down min_u max_v) DS.|> quick_create_vertex color right down max_u max_v DS.|> quick_create_vertex color right up max_u min_v DS.|> quick_create_vertex color left up min_u min_v,index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter point.x point.y matrix maybe_border,vertex_size=4,index_size=6}
    Animation {arrange,half_width,half_height,reciprocal_width,reciprocal_height,padding,width_number,height_number,album_number,album_id,index}->case arrange_transform arrange of
        Arrange {point,matrix,color}->case point of
            Point {x,y}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in let (quotient,remainder)=divMod index (width_number*height_number) in let new_album_id=album_id+quotient in if album_number<=quotient then EF.empty_error else let (new_quotient,new_remainder)=divMod remainder width_number in let frame_x=2*fromIntegral new_remainder*(half_width+padding)+padding in let frame_y=2*fromIntegral new_quotient*(half_height+padding)+padding in let min_u=frame_x*reciprocal_width in let min_v=frame_y*reciprocal_height in let max_u=(frame_x+2*half_width)*reciprocal_width in let max_v=(frame_y+2*half_height)*reciprocal_height in Submit {submit_mode=Submit_album {album_id=new_album_id},vertex=DS.singleton (quick_create_vertex color left down min_u max_v) DS.|> quick_create_vertex color right down max_u max_v DS.|> quick_create_vertex color right up max_u min_v DS.|> quick_create_vertex color left up min_u min_v,index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter x y matrix maybe_border,vertex_size=4,index_size=6}
    ...
    Canvas {arrange,half_width,half_height,canvas_id}->case arrange_transform arrange of
        Arrange {point,matrix,color}->case point of
            Point {x,y}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in Submit {submit_mode=Submit_canvas {canvas_id=canvas_id},vertex=DS.singleton (quick_create_vertex color left down 0 1) DS.|> quick_create_vertex color right down 1 1 DS.|> quick_create_vertex color right up 1 0 DS.|> quick_create_vertex color left up 0 0,index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=to_Parameter x y matrix maybe_border,vertex_size=4,index_size=6}
