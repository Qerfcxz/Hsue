...
to_collect u v maybe_border widget=case widget of
    Visual {origin,matrix,red,green,blue,alpha,visual}->let parameter=to_Parameter origin matrix maybe_border in case visual of
        ...
        Picture {half_width,half_height,min_u,min_v,max_u,max_v,locked}->if locked then EE.quick_error "to_collect" 2 else case origin of
        ...
        Large_picture {half_width,half_height,album_id}->case origin of
        ...
        Atlas {clip,index,locked}->if locked then EE.quick_error "to_collect" 3 else case origin of
            Point {x=origin_x,y=origin_y}->case clip DVS.! index of
                Clip {x,y,half_width,half_height,min_u,min_v,max_u,max_v}->let new_x=origin_x+x in let new_y=origin_y+y in let left=new_x-half_width in let down=new_y-half_height in let right=new_x+half_width in let up=new_y+half_height in Submit {maybe_canvas_id=Nothing,maybe_album_id=Nothing,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=min_u,v=max_v,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=max_u,v=max_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=max_u,v=min_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=min_u,v=min_v,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
        Large_atlas {clip,album_id,index}->case origin of
            Point {x=origin_x,y=origin_y}->case clip DVS.! index of
                Clip {x,y,half_width,half_height,min_u,min_v,max_u,max_v}->let new_x=origin_x+x in let new_y=origin_y+y in let left=new_x-half_width in let down=new_y-half_height in let right=new_x+half_width in let up=new_y+half_height in Submit {maybe_canvas_id=Nothing,maybe_album_id=Just album_id,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=min_u,v=max_v,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=max_u,v=max_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=max_u,v=min_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=min_u,v=min_v,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
        Animation {half_width,half_height,reciprocal_width,reciprocal_height,padding,width_number,height_number,album_number,album_id,index}->case origin of
            Point {x,y}->let left=x-half_width in let down=y-half_height in let right=x+half_width in let up=y+half_height in let (quotient,remainder)=divMod index (width_number*height_number) in let new_album_id=album_id+quotient in if album_number<=quotient then EE.quick_error "to_collect" 4 else let (new_quotient,new_remainder)=divMod remainder width_number in let frame_x=2*fromIntegral new_remainder*(half_width+padding)+padding in let frame_y=2*fromIntegral new_quotient*(half_height+padding)+padding in let min_u=frame_x*reciprocal_width in let min_v=frame_y*reciprocal_height in let max_u=(frame_x+2*half_width)*reciprocal_width in let max_v=(frame_y+2*half_height)*reciprocal_height in Submit {maybe_canvas_id=Nothing,maybe_album_id=Just new_album_id,vertex=DS.singleton (Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=down,u=min_u,v=max_v,parameter_id=0,size=0}) DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=down,u=max_u,v=max_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=right,y=up,u=max_u,v=min_v,parameter_id=0,size=0} DS.|> Vertex {red=red,green=green,blue=blue,alpha=alpha,x=left,y=up,u=min_u,v=min_v,parameter_id=0,size=0},index=DS.singleton 0 DS.|> 1 DS.|> 2 DS.|> 0 DS.|> 2 DS.|> 3,parameter=parameter,vertex_length=4,index_length=6}
        Canvas {half_width,half_height,canvas_id,locked}->if locked then EE.quick_error "to_collect" 5 else case origin of
    ...
    Text {origin,matrix,half_width,half_height,y,article}->case origin of
        Point {x=origin_x,y=origin_y}->case maybe_border of
            Nothing->let (vertex,index,_)=DF.foldl' (DF.foldl' (flip (collect_text origin_x origin_y y))) (DS.empty,DS.empty,0) (fmap (DS.takeWhileL (end_text y half_height) . DS.dropWhileL (begin_text y half_height)) article) in Submit {maybe_canvas_id=Nothing,maybe_album_id=Nothing,vertex=vertex,index=index,parameter=Parameter {x=origin_x+matrix.x,y=origin_y+matrix.y,x_x=matrix.x_x,x_y=matrix.x_y,y_x=matrix.y_x,y_y=matrix.y_y,border_flag=1,border_left=origin_x-half_width,border_down=origin_y-half_height,border_right=origin_x+half_width,border_up=origin_y+half_height},vertex_length=fromIntegral (DS.length vertex),index_length=fromIntegral (DS.length index)}
            Just border->case border of
                Border {left,down,right,up}->let border_down=max (-half_height) down in let border_up=min half_height up in let (vertex,index,_)=DF.foldl' (DF.foldl' (flip (collect_text origin_x origin_y y))) (DS.empty,DS.empty,0) (fmap (DS.takeWhileL (end_text y (-border_down)) . DS.dropWhileL (begin_text y border_up)) article) in Submit {maybe_canvas_id=Nothing,maybe_album_id=Nothing,vertex=vertex,index=index,parameter=Parameter {x=origin_x+matrix.x,y=origin_y+matrix.y,x_x=matrix.x_x,x_y=matrix.x_y,y_x=matrix.y_x,y_y=matrix.y_y,border_flag=1,border_left=origin_x+max (-half_width) left,border_down=origin_y+border_down,border_right=origin_x+min half_width right,border_up=origin_y+border_up},vertex_length=fromIntegral (DS.length vertex),index_length=fromIntegral (DS.length index)}
    Custom_widget {custom}->custom_widget_collect u v maybe_border custom
    _->EE.quick_error "to_collect" 6
