...
first_maybe_update_collect::(Widget a->Maybe (Widget a))->Projection_path->Int->Collect_strategy->Engine a->Engine a
first_maybe_update_collect update projection_path leaf_id collect_strategy engine=let maybe_tuple=DFC.getCompose (functor_lookup_projection_widget projection_path (\widget->DFC.Compose {getCompose=fmap (\this_widget->(to_collect engine.u engine.v this_widget,this_widget)) (update widget)}) engine) in case maybe_tuple of
    Nothing->engine
...
second_maybe_update_collect::(Widget a->Maybe (Widget a))->Projection_path->Int->Collect_strategy->Engine a->Engine a
second_maybe_update_collect update projection_path leaf_id collect_strategy engine=let (new_update,maybe_engine)=DFC.getCompose (functor_lookup_projection_widget projection_path (\widget->DFC.Compose {getCompose=(intmap_update leaf_id (update_projection_object (collect_a (DS.singleton (to_collect engine.u engine.v widget)) collect_strategy)),update widget)}) engine) in case maybe_engine of
    Nothing->engine
...
collect_a seq_submit collect_strategy this_widget=case this_widget of
    Double {which,first_widget,second_widget}->if which then Double {which=which,first_widget=collect_a seq_submit collect_strategy first_widget,second_widget=second_widget} else Double {which=which,first_widget=first_widget,second_widget=collect_a seq_submit collect_strategy second_widget}
    Group {index,group_widget}->Group {index=index,group_widget=intmap_update index (collect_a seq_submit collect_strategy) group_widget}
    Widget_trigger {next,widget_trigger,widget}->Widget_trigger {next=next,widget_trigger=widget_trigger,widget=collect_a seq_submit collect_strategy widget}
    Widget_io_trigger {next,widget_io_trigger,widget}->Widget_io_trigger {next=next,widget_io_trigger=widget_io_trigger,widget=collect_a seq_submit collect_strategy widget}
    Widget_mix_trigger {next,widget_mix_trigger,order,widget}->Widget_mix_trigger {next=next,widget_mix_trigger=widget_mix_trigger,order=order,widget=collect_a seq_submit collect_strategy widget}
    Collector {initial_min_index,initial_max_index,min_index,max_index,submit}->case collect_strategy of
        Min_strategy->Collector {initial_min_index=initial_min_index,initial_max_index=initial_max_index,min_index=min_index-1,max_index=max_index,submit=intmap_insert min_index seq_submit submit}
        Max_strategy->Collector {initial_min_index=initial_min_index,initial_max_index=initial_max_index,min_index=min_index,max_index=max_index+1,submit=intmap_insert max_index seq_submit submit}
        Index_strategy {seat}->if seat<=min_index then Collector {initial_min_index=initial_min_index,initial_max_index=initial_max_index,min_index=seat-1,max_index=max_index,submit=intmap_insert seat seq_submit submit} else if max_index<=seat then Collector {initial_min_index=initial_min_index,initial_max_index=initial_max_index,min_index=min_index,max_index=seat+1,submit=intmap_insert seat seq_submit submit} else Collector {initial_min_index=initial_min_index,initial_max_index=initial_max_index,min_index=min_index,max_index=max_index,submit=intmap_insert seat seq_submit submit}
    _->error "collect_a: error 1"

to_collect::FCT.CFloat->FCT.CFloat->Widget a->Submit
to_collect u v widget=let new_widget=get_widget widget in case new_widget of
...
move_a widget=let new_widget=get_widget widget in case new_widget of
...
move_lookup projection_move engine=case projection_move of
    Object_move {leaf_id,consume}->if consume then let (widget,leaf)=intmap_functor_update leaf_id (DT.swap . update_lookup_projection_widget_a consume_widget) engine.leaf in (engine {leaf=leaf},widget) else (engine,lookup_projection_object (intmap_lookup leaf_id engine.leaf))
...
consume_widget this_widget=case this_widget of
    Double {which,first_widget,second_widget}->if which then Double {which=which,first_widget=consume_widget first_widget,second_widget=second_widget} else Double {which=which,first_widget=first_widget,second_widget=consume_widget second_widget}
    Group {index,group_widget}->Group {index=index,group_widget=intmap_update index consume_widget group_widget}
    Widget_trigger {next,widget_trigger,widget}->Widget_trigger {next=next,widget_trigger=widget_trigger,widget=consume_widget widget}
    Widget_io_trigger {next,widget_io_trigger,widget}->Widget_io_trigger {next=next,widget_io_trigger=widget_io_trigger,widget=consume_widget widget}
    Widget_mix_trigger {next,widget_mix_trigger,order,widget}->Widget_mix_trigger {next=next,widget_mix_trigger=widget_mix_trigger,order=order,widget=consume_widget widget}
    Collector {initial_min_index,initial_max_index}->Collector {initial_min_index=initial_min_index,initial_max_index=initial_max_index,min_index=initial_min_index,max_index=initial_max_index,submit=DIM.empty}
