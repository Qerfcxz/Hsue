...
create_node node_id maybe_father_id event_transform widget_transform engine=case maybe_father_id of
    ...
    Just father_id->let (new_node,node)=intmap_update_lookup father_id (\this_node->this_node {node_child=intset_insert node_id this_node.node_child}) engine.node in engine {node=intmap_insert node_id (Node {ancestry_id=node.ancestry_id DS.|> father_id,leaf_child=DIS.empty,node_child=DIS.empty,event_transform=event_transform,widget_transform=widget_transform}) new_node}

remove_node::Int->Engine a->IO (Engine a)
remove_node node_id engine=let (new_node,node)=intmap_delete_lookup node_id engine.node in case node.ancestry_id of
    DS.Empty->remove_node_a node.leaf_child node.node_child (engine {node=new_node})
    _ DS.:|> father_id->remove_node_a node.leaf_child node.node_child (engine {node=intmap_update father_id (\this_node->this_node {node_child=intset_delete node_id this_node.node_child}) new_node})
