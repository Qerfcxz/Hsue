...
data Request a b c d e=Reset_timer {interval::DW.Word64}|Stop_timer|Stop_timer_safe|Create_widget {leaf_id::Int,maybe_father_id::Maybe Int,widget_request::Widget_request a b c d e}|Remove_widget {leaf_id::Int}|Create_node {node_id::Int,maybe_father_id::Maybe Int,event_transform::Engine a b c d e->Event b->Event b,widget_transform::Event b->Engine a b c d e->Widget a b c d e->Widget a b c d e}|Remove_node {node_id::Int}|Create_window {window_id::Int,title::DT.Text,window_width::FCT.CInt,window_height::FCT.CInt,red::FCT.CFloat,green::FCT.CFloat,blue::FCT.CFloat,alpha::FCT.CFloat,window_flag::DSet.Set Window_flag}|Remove_window {window_id::Int}|Create_canvas {canvas_width::DW.Word32,canvas_height::DW.Word32,maybe_canvas_id::Maybe Int}|Remove_canvas {canvas_id::Int}|Clean_atlas|Unlock {leaf_id::Int}|Load_charset {charset::DM.Map String (DSet.Set Char)}|Render {window_id::Int,render_selector::Selector (),projection_move::Projection_move}|Canvas_render {canvas_id::Int,canvas_render_selector::Selector (),projection_move::Projection_move}|Canvas_widget_render {projection_path::Projection_path,canvas_widget_render_selector::Selector (Selector ()),projection_move::Projection_move}|Io {io::Engine a b c d e->IO (Engine a b c d e)}|Custom_request {custom::c}
...
data Canvas=Free_canvas {width::DW.Word32,height::DW.Word32,half_width::FCT.CFloat,half_height::FCT.CFloat,texture::FP.Ptr SDLT.SDL_GPUTexture}|Bound_canvas {texture::FP.Ptr SDLT.SDL_GPUTexture,leaf_id::Int}
