...
from_same_insert_widget::ET.Has_call_stack=>Int->DS.Seq Insert_strategy->Widget a b c d e->Engine a b c d e->Engine a b c d e
...
from_same_insert_widget_a::ET.Has_call_stack=>DS.Seq Insert_strategy->Widget a b c d e->Widget a b c d e->Widget a b c d e
...
from_insert_widget::ET.Has_call_stack=>Int->DS.Seq (Insert (Widget a b c d e))->Engine a b c d e->Engine a b c d e
...
from_insert_widget_a::ET.Has_call_stack=>DS.Seq (Insert (Widget a b c d e))->Widget a b c d e->Widget a b c d e
...
from_insert_widget_b::ET.Has_call_stack=>Int->Int->(Widget a b c d e->f)->DS.Seq (Insert (Widget a b c d e))->DIM.IntMap f->(DIM.IntMap f,Int,Int)
...
create_leaf::ET.Has_call_stack=>Custom_widget_request e=>Int->Maybe Int->Widget_request a b c d e->Engine a b c d e->IO (Engine a b c d e)
...
create_widget::ET.Has_call_stack=>Custom_widget_request e=>Int->Widget_request a b c d e->Engine a b c d e->IO (Engine a b c d e,Widget a b c d e)
...
from_insert_widget_request::ET.Has_call_stack=>Custom_widget_request e=>Int->Int->Int->(Widget a b c d e->f)->DS.Seq (Insert (Widget_request a b c d e))->DIM.IntMap f->Engine a b c d e->IO (Engine a b c d e,DIM.IntMap f,Int,Int)
...
create_visual::ET.Has_call_stack=>Visual_request->Engine a b c d e->IO (Engine a b c d e,Visual)
create_visual visual_request engine=case visual_request of
    ...
    Text_request {arrange,text_width,text_height,article,calculate_width,calculate_typesetting,load}->let charset=to_charset article in let half_height=text_height/2 in if load
        then do
            ...
            return (new_engine,let (new_article,number)=for_text new_engine.font new_engine.font_map article calculate_width in let (new_new_article,max_y)=do_typesetting number half_height calculate_typesetting new_article in Text {arrange=arrange,half_width=text_width/2,half_height=half_height,current_y=0,min_y=0,max_y=max_y-half_height,article=new_new_article,charset=charset,locked=False})
        else return (engine,let (new_article,number)=for_text engine.font engine.font_map article calculate_width in let (new_new_article,max_y)=do_typesetting number half_height calculate_typesetting new_article in Text {arrange=arrange,half_width=text_width/2,half_height=half_height,current_y=0,min_y=0,max_y=max_y-half_height,article=new_new_article,charset=charset,locked=False})
...
create_picture::ET.Has_call_stack=>Arrange->String->Engine a b c d e->IO (Engine a b c d e,Visual)
...
create_atlas::ET.Has_call_stack=>Arrange->DS.Seq Clip_request->String->Int->Engine a b c d e->IO (Engine a b c d e,Visual)
...
remove_leaf::ET.Has_call_stack=>Custom_widget d=>Int->Engine a b c d e->IO (Engine a b c d e)
...
remove_leaf_a::ET.Has_call_stack=>Custom_widget d=>DS.Seq Int->Widget a b c d e->DIM.IntMap (Projection a b c d e)->Int->Engine a b c d e->IO (Engine a b c d e)
...
remove_widget::ET.Has_call_stack=>Custom_widget d=>Widget a b c d e->Engine a b c d e->IO (Engine a b c d e)
...
remove_visual::ET.Has_call_stack=>Visual->Engine a b c d e->IO (Engine a b c d e)
...
create_node::ET.Has_call_stack=>Int->Maybe Int->(Engine a b c d e->Event b->Event b)->(Event b->Engine a b c d e->Widget a b c d e->Widget a b c d e)->Engine a b c d e->Engine a b c d e
...
remove_node::ET.Has_call_stack=>Custom_widget d=>Int->Engine a b c d e->IO (Engine a b c d e)
...
remove_node_a::ET.Has_call_stack=>Custom_widget d=>DIS.IntSet->DIS.IntSet->Engine a b c d e->IO (Engine a b c d e)
...
remove_node_leaf::ET.Has_call_stack=>Custom_widget d=>Int->Engine a b c d e->IO (Engine a b c d e)
...
remove_node_node::ET.Has_call_stack=>Custom_widget d=>Int->Engine a b c d e->IO (Engine a b c d e)
