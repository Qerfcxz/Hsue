...
create_bound father widget_request bound_id engine=do
    (widget,window_id,resource)<-make_bound widget_request
    let (maybe_window,new_window)=DIM.updateLookupWithKey (\_->just_update (\window->window {window_bound=intset_insert bound_id window.window_bound})) window_id engine.window in case maybe_window of
        ...
        Just _->case father of
            Nothing->return (engine {bound=intmap_insert bound_id (Bound {resource=resource,window_id=window_id,ancestry=DS.empty,widget=widget}) engine.bound,window=new_window})
            Just node_id->let (maybe_node,new_node)=DIM.updateLookupWithKey (\_->just_update (\node->node {bound_child=intset_insert bound_id node.bound_child})) node_id engine.node in case maybe_node of
                ...
                Just node->return (engine {bound=intmap_insert bound_id (Bound {resource=resource,window_id=window_id,ancestry=node.ancestry DS.|> node_id,widget=widget}) engine.bound,node=new_node,window=new_window})

make_bound::Widget_request a->IO (Widget a,Int,Resource)
make_bound widget_request=case widget_request of
    Geometry_request {window_id,red,green,blue,alpha,geometry}->return (Geometry {red,green,blue,alpha,geometry},window_id,None_resource)
...
remove_active active_id engine=let (maybe_active,new_active)=DIM.updateLookupWithKey (\_ _->Nothing) active_id engine.active in case maybe_active of
    ...
    Just active->case active.ancestry of
...
remove_free free_id engine=let (maybe_free,new_free)=DIM.updateLookupWithKey (\_ _->Nothing) free_id engine.free in case maybe_free of
    ...
    Just free->case free.ancestry of
...
clean_resource::Resource->IO ()
clean_resource resource=case resource of
    None_resource->return ()