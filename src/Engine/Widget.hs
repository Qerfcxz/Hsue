...
create_active active_id father widget_request engine=let (widget,next)=make_active widget_request in case father of
    Nothing->engine {active=intmap_insert active_id (Active {ancestry=DS.empty,projection=Without {object=widget},next=next}) engine.active}
    Just node_id->let (new_node,node)=intmap_update_lookup node_id (\this_node->this_node {active_child=intset_insert active_id this_node.active_child}) engine.node in engine {active=intmap_insert active_id (Active {ancestry=node.ancestry DS.|> node_id,projection=Without {object=widget},next=next}) engine.active,node=new_node}
...
make_active widget_request=case widget_request of
    ...
    Mix_trigger_request {mix_trigger,order,next}->(Mix_trigger {mix_trigger=mix_trigger,order=order},next)
    ...
    Int_mix_trigger_request {int_mix_trigger,order,int,next}->(Int_mix_trigger {int_mix_trigger=int_mix_trigger,order=order,int=int},next)
...
create_inactive inactive_id father widget_request engine=do
    ...
    case father of
        Nothing->return (new_engine {inactive=intmap_insert inactive_id (Inactive {ancestry=DS.empty,projection=Without {object=widget}}) new_engine.inactive})
        Just node_id->let (new_node,node)=intmap_update_lookup node_id (\this_node->this_node {inactive_child=intset_insert inactive_id this_node.inactive_child}) new_engine.node in return (new_engine {inactive=intmap_insert inactive_id (Inactive {ancestry=node.ancestry DS.|> node_id,projection=Without {object=widget}}) new_engine.inactive,node=new_node})
...
create_inactive_int_trigger_request::(Widget a->Int->Event->Engine a->(Engine a->Engine a,Int,Widget a))->Int->Int->(Engine a->Event->Maybe Int)->Widget_request a
create_inactive_int_trigger_request transform inactive_id int next=Int_trigger_request {int_trigger=create_inactive_int_trigger_request_a transform inactive_id,int=int,next=next}

create_inactive_int_trigger_request_a::(Widget a->Int->Event->Engine a->(Engine a->Engine a,Int,Widget a))->Int->Int->Event->Engine a->(Engine a->Engine a,Int)
create_inactive_int_trigger_request_a transform inactive_id int event engine=let (update,new_int,inactive)=intmap_functor_update inactive_id (functor_update_inactive_projection (functor_update_object (\widget->transform widget int event engine))) engine.inactive in (update . \this_engine->this_engine {inactive=inactive},new_int)

create_inactive_int_mix_trigger_request::(Widget a->Int->Event->Engine a->(Engine a->IO (Engine a),Int,Widget a))->Int->Int->(Engine a->Event->Maybe Int)->Widget_request a
create_inactive_int_mix_trigger_request transform inactive_id int next=Int_mix_trigger_request {int_mix_trigger=create_inactive_int_mix_trigger_request_a transform inactive_id,order=True,int=int,next=next}

create_inactive_int_mix_trigger_request_a::(Widget a->Int->Event->Engine a->(Engine a->IO (Engine a),Int,Widget a))->Int->Int->Event->Engine a->(Engine a->Engine a,Engine a->IO (Engine a),Int)
create_inactive_int_mix_trigger_request_a transform inactive_id int event engine=let (update,new_int,inactive)=intmap_functor_update inactive_id (functor_update_inactive_projection (functor_update_object (\widget->transform widget int event engine))) engine.inactive in (\this_engine->this_engine {inactive=inactive},update,new_int)