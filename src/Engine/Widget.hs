...
make_inactive widget_request engine=case widget_request of
    ...
    Text_request {origin,matrix,width,height,article,calculate_width,calculate_typesetting}->return (engine,Text {origin=origin,matrix=matrix,width=width,height=height,y=0,article=do_typesetting (height/2) calculate_typesetting (for_text engine.font article calculate_width)})
