...
create_triangle_graphics_pipeline sdl_window device vertex_shader fragment_shader=FMA.allocaArray 3 $ \vertex_attribute->do
    ...
    FMU.with C.SDL_GPUVertexBufferDescription {sdl_slot=0,sdl_pitch=32,sdl_input_rate=C.sdl_gpu_vertexinputrate_vertex,sdl_instance_step_rate=0} $ \vertex_buffer_description->do
...
load_shader device format stage num_sampler num_uniform_buffer path=do
    ...
    DBS.useAsCStringLen shader_code (\(code,code_size)->FCS.withCString "main" (\entrypoint->FMU.with (C.SDL_GPUShaderCreateInfo {sdl_code_size=fromIntegral code_size,sdl_code=FP.castPtr code,sdl_entrypoint=entrypoint,sdl_format=format,sdl_stage=stage,sdl_num_samplers=num_sampler,sdl_num_storage_textures=0,sdl_num_storage_buffers=0,sdl_num_uniform_buffers=num_uniform_buffer}) (return_catch_null . F.sdl_creategpushader device)))