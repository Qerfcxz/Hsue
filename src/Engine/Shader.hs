...
create_triangle_graphics_pipeline sdl_window device vertex_shader fragment_shader=FMA.allocaArray 3 $ \vertex_attribute->do
    ...
    FS.pokeElemOff vertex_attribute 2 C.SDL_GPUVertexAttribute {location=2,buffer_slot=0,format=C.sdl_gpu_vertexelementformat_float2,offset=24}
    FMU.with C.SDL_GPUVertexBufferDescription {slot=0,pitch=32,input_rate=C.sdl_gpu_vertexinputrate_vertex,instance_step_rate=0} $ \vertex_buffer_description->do
        format<-F.sdl_getgpuswapchaintextureformat device sdl_window
        FMU.with C.SDL_GPUColorTargetDescription {format=format,blend_state=standard_blend_state} (\color_target_description->FMU.with C.SDL_GPUGraphicsPipelineCreateInfo {vertex_shader=vertex_shader,fragment_shader=fragment_shader,vertex_input_state=C.SDL_GPUVertexInputState {vertex_buffer_descriptions=vertex_buffer_description,num_vertex_buffers=1,vertex_attributes=vertex_attribute,num_vertex_attributes=3},primitive_type=C.sdl_gpu_primitivetype_trianglelist,target_info=C.SDL_GPUGraphicsPipelineTargetInfo {color_target_descriptions=color_target_description,num_color_targets=1,has_depth_stencil_target=FMU.fromBool False}} (return_catch_null . F.sdl_creategpugraphicspipeline device))
...
update_buffer device command_buffer vertex_buffer index_buffer transfer_buffer vertex_size index_size vertex index=let vertex_length=DS.length vertex in let index_length=DS.length index in if vertex_length==0||index_length==0 then return Nothing else let new_vertex_size=vertex_length*FS.sizeOf (undefined::Vertex) in let new_index_size=index_length*FS.sizeOf (undefined::DW.Word32) in if vertex_size<new_vertex_size||index_size<new_index_size then error "update_buffer: error 1" else do
    ...
    FMU.with (C.SDL_GPUTransferBufferLocation {transfer_buffer=transfer_buffer,offset=0}) (\transfer_buffer_location->FMU.with (C.SDL_GPUBufferRegion {buffer=vertex_buffer,offset=0,size=fromIntegral new_vertex_size}) (\buffer_region->F.sdl_uploadtogpubuffer copy_pass transfer_buffer_location buffer_region (FMU.fromBool True)))
    FMU.with (C.SDL_GPUTransferBufferLocation {transfer_buffer=transfer_buffer,offset=fromIntegral vertex_size}) (\transfer_buffer_location->FMU.with (C.SDL_GPUBufferRegion {buffer=index_buffer,offset=0,size=fromIntegral new_index_size}) (\buffer_region->F.sdl_uploadtogpubuffer copy_pass transfer_buffer_location buffer_region (FMU.fromBool True)))
...
load_shader device format stage num_sampler num_uniform_buffer path=do
    ...
    DBS.useAsCStringLen shader (\(code,code_size)->FCS.withCString "main" (\entrypoint->FMU.with (C.SDL_GPUShaderCreateInfo {code_size=fromIntegral code_size,code=FP.castPtr code,entrypoint=entrypoint,format=format,stage=stage,num_samplers=num_sampler,num_storage_textures=0,num_storage_buffers=0,num_uniform_buffers=num_uniform_buffer}) (return_catch_null . F.sdl_creategpushader device)))