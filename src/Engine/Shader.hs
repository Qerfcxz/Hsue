...
load_shader device format stage path=do
    ...
    DBS.useAsCStringLen shader (\(ptr,size)->FCS.withCString "main" (\cstring->FMU.with (C.SDL_GPUShaderCreateInfo {code_size=fromIntegral size,code=FP.castPtr ptr,entrypoint=cstring,stage=stage,format=format}) (F.sdl_creategpushader device)))