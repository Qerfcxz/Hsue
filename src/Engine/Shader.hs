...
update_buffer device command_buffer vertex_buffer index_buffer widget_parameter_buffer transfer_buffer vertex_size index_size widget_parameter_size vertex index widget_parameter=let vertex_length=DS.length vertex in let index_length=DS.length index in let widget_parameter_length=DS.length widget_parameter in if vertex_length==0||index_length==0||widget_parameter_length==0 then return Nothing else let single_vertex=FS.sizeOf (undefined::Vertex) in let single_index=FS.sizeOf (undefined::DW.Word32) in let single_widget_parameter=FS.sizeOf (undefined::Parameter) in let new_vertex_size=vertex_length*single_vertex in let new_index_size=index_length*single_index in let new_widget_parameter_size=widget_parameter_length*single_widget_parameter in if vertex_size<new_vertex_size||index_size<new_index_size||widget_parameter_size<new_widget_parameter_size then error "update_buffer: error 1" else do
    ...
    seq_poke_array single_widget_parameter widget_parameter (FP.plusPtr map_transfer_buffer size)
    ...
    FMU.with (C.SDL_GPUTransferBufferLocation {sdl_transfer_buffer=transfer_buffer,sdl_offset=fromIntegral size}) (\transfer_buffer_location->FMU.with (C.SDL_GPUBufferRegion {sdl_buffer=widget_parameter_buffer,sdl_offset=0,sdl_size=fromIntegral new_widget_parameter_size}) (\buffer_region->F.sdl_uploadtogpubuffer copy_pass transfer_buffer_location buffer_region (FMU.fromBool True)))
