...
run_event leaf_id event engine=case DIM.lookup leaf_id engine.leaf of
    ...
    Just projection->let new_event=DF.foldl' (\this_event node_id->(int_map_lookup node_id engine.node).event_transform engine this_event) event (lookup_projection_ancestry_id projection) in let (next,update,new_projection)=run_event_a new_event (`insert_projection_object` projection) (trigger_selector_applicative_update engine.strict_exist (run_widget engine.strict_match new_event engine) (lookup_projection (engine.projection_strategy event engine) projection)) in let new_engine=update (engine {leaf=DIM.insert leaf_id new_projection engine.leaf}) in maybe new_engine (\this_leaf_id->run_event this_leaf_id event new_engine) (next new_engine)
