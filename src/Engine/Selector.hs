...
selector_action action this_selector this_widget engine=case this_selector of
    ...
    Default_selector {maybe_value,value,bounded}->default_selector_action bounded (action value) this_widget (self_selector_action maybe_value action this_widget engine)
    Hosted_selector {maybe_value,selector,bounded,strict}->case this_widget of
        Group {index,group_widget}->let backup=self_selector_action maybe_value action this_widget engine in group_default_selector_action bounded (const backup) (selector_action action selector) index group_widget backup
        Vector {index,vector_widget}->let backup=self_selector_action maybe_value action this_widget engine in vector_default_selector_action bounded (const backup) (selector_action action selector) index vector_widget backup
        ...
        Coroutine {index,coroutine_state}->let backup=self_selector_action maybe_value action this_widget engine in coroutine_default_selector_action bounded (const backup) (selector_action action selector) index coroutine_state backup
    ...
    Any_selector {maybe_value,selector,strict}->case this_widget of
        Group {group_widget}->group_all_selector_action DIM.foldl' (selector_action action selector) group_widget (self_selector_action maybe_value action this_widget engine)
        Vector {vector_widget}->vector_all_selector_action DF.foldl' (selector_action action selector) vector_widget (self_selector_action maybe_value action this_widget engine)
        Widget_trigger {widget}->widget_trigger_selector_action (selector_action action selector) widget (self_selector_action maybe_value action this_widget engine)
        Widget_io_trigger {widget}->widget_trigger_selector_action (selector_action action selector) widget (self_selector_action maybe_value action this_widget engine)
        Widget_mix_trigger {widget}->widget_trigger_selector_action (selector_action action selector) widget (self_selector_action maybe_value action this_widget engine)
        Coroutine {coroutine_state}->coroutine_all_selector_action DIM.foldl' (selector_action action selector) coroutine_state (self_selector_action maybe_value action this_widget engine)
...
selector_monad_action action this_selector this_widget engine=case this_selector of
    ...
    Default_selector {maybe_value,value,bounded}->do
        new_engine<-self_selector_applicative_action maybe_value action this_widget engine
        default_selector_monad_action bounded (action value) this_widget new_engine
    Hosted_selector {maybe_value,selector,bounded,strict}->case this_widget of
        Group {index,group_widget}->do
            new_engine<-self_selector_applicative_action maybe_value action this_widget engine
            group_default_selector_action bounded return (selector_monad_action action selector) index group_widget new_engine
        Vector {index,vector_widget}->do
            new_engine<-self_selector_applicative_action maybe_value action this_widget engine
            vector_default_selector_action bounded return (selector_monad_action action selector) index vector_widget new_engine
        ...
        Coroutine {index,coroutine_state}->do
            new_engine<-self_selector_applicative_action maybe_value action this_widget engine
            coroutine_default_selector_action bounded return (selector_monad_action action selector) index coroutine_state new_engine
    ...
    Any_selector {maybe_value,selector,strict}->case this_widget of
        Group {group_widget}->do
            new_engine<-self_selector_applicative_action maybe_value action this_widget engine
            group_all_selector_action DF.foldlM (selector_monad_action action selector) group_widget new_engine
        Vector {vector_widget}->do
            new_engine<-self_selector_applicative_action maybe_value action this_widget engine
            vector_all_selector_action DF.foldlM (selector_monad_action action selector) vector_widget new_engine
        Widget_trigger {widget}->do
            new_engine<-self_selector_applicative_action maybe_value action this_widget engine
            selector_monad_action action selector widget new_engine
        Widget_io_trigger {widget}->do
        ...
        Coroutine {coroutine_state}->do
            new_engine<-self_selector_applicative_action maybe_value action this_widget engine
            coroutine_all_selector_action DF.foldlM (selector_monad_action action selector) coroutine_state new_engine
...
selector_update update this_selector this_widget=case this_selector of
    ...
    Default_selector {maybe_value,value,bounded}->self_selector_update maybe_value update (default_selector_update bounded (update value) this_widget)
    Hosted_selector {maybe_value,selector,bounded,strict}->case this_widget of
        Group {initial_min_index,min_index,initial_max_index,max_index,index,group_widget}->self_selector_update maybe_value update (group_default_selector_update bounded this_widget id (\this_index this_this_selector this_group_widget->(if bounded then intmap_update else intmap_update_safe) this_index (selector_update update this_this_selector) this_group_widget) selector initial_min_index min_index initial_max_index max_index index group_widget)
        Vector {index,size,vector_widget}->self_selector_update maybe_value update (vector_default_selector_update bounded this_widget id (\this_index this_this_selector this_vector_widget->DV.modify (\this_this_vector_widget->DVM.write this_this_vector_widget this_index (selector_update update this_this_selector (this_vector_widget DV.! this_index))) this_vector_widget) selector index size vector_widget)
        Widget_trigger {next,widget_trigger,widget}->self_selector_update maybe_value update (widget_trigger_selector_update id id (selector_update update selector) next widget_trigger widget)
        Widget_io_trigger {next,widget_io_trigger,widget}->self_selector_update maybe_value update (widget_io_trigger_selector_update id id (selector_update update selector) next widget_io_trigger widget)
        Widget_mix_trigger {next,widget_mix_trigger,order,widget}->self_selector_update maybe_value update (widget_mix_trigger_selector_update id id (selector_update update selector) next widget_mix_trigger order widget)
        Coroutine {index,initial_min_index,min_index,initial_max_index,max_index,variable_length,user_variable_length,coroutine_state,layout,linear_coroutine,iterative}->self_selector_update maybe_value update (coroutine_default_selector_update bounded this_widget id (\this_index this_this_selector this_coroutine_state->(if bounded then intmap_update else intmap_update_safe) this_index (update_coroutine_state (selector_update update this_this_selector)) this_coroutine_state) selector index initial_min_index min_index initial_max_index max_index variable_length user_variable_length layout linear_coroutine iterative coroutine_state)
        _->if strict then EE.quick_error "selector_update" 0 else self_selector_update maybe_value update this_widget
    Any_selector {maybe_value,selector,strict}->case this_widget of
        Group {initial_min_index,min_index,initial_max_index,max_index,index,group_widget}->self_selector_update maybe_value update (group_all_selector_update id fmap (selector_update update selector) initial_min_index min_index initial_max_index max_index index group_widget)
        Vector {index,size,vector_widget}->self_selector_update maybe_value update (vector_all_selector_update id fmap (selector_update update selector) index size vector_widget)
        Widget_trigger {next,widget_trigger,widget}->self_selector_update maybe_value update (widget_trigger_selector_update id id (selector_update update selector) next widget_trigger widget)
        Widget_io_trigger {next,widget_io_trigger,widget}->self_selector_update maybe_value update (widget_io_trigger_selector_update id id (selector_update update selector) next widget_io_trigger widget)
        Widget_mix_trigger {next,widget_mix_trigger,order,widget}->self_selector_update maybe_value update (widget_mix_trigger_selector_update id id (selector_update update selector) next widget_mix_trigger order widget)
        Coroutine {index,initial_min_index,min_index,initial_max_index,max_index,variable_length,user_variable_length,coroutine_state,layout,linear_coroutine,iterative}->self_selector_update maybe_value update (coroutine_all_selector_update id fmap (update_coroutine_state (selector_update update selector)) index initial_min_index min_index initial_max_index max_index variable_length user_variable_length layout linear_coroutine iterative coroutine_state)
...
selector_monad_update update this_selector this_widget=case this_selector of
    ...
    Trigger_selector {maybe_value,value,bounded}->do
        new_widget<-trigger_selector_applicative_update bounded (update value) this_widget
        self_selector_applicative_update maybe_value update new_widget
    Default_selector {maybe_value,value,bounded}->do
        new_widget<-default_selector_applicative_update bounded (update value) this_widget
        self_selector_applicative_update maybe_value update new_widget
    Hosted_selector {maybe_value,selector,bounded,strict}->case this_widget of
        Group {initial_min_index,min_index,initial_max_index,max_index,index,group_widget}->do
            new_widget<-group_default_selector_update bounded (return this_widget) fmap (\this_index this_this_selector this_group_widget->intmap_functor_update this_index (selector_monad_update update this_this_selector) this_group_widget) selector initial_min_index min_index initial_max_index max_index index group_widget
            self_selector_applicative_update maybe_value update new_widget
        Vector {index,size,vector_widget}->do
            new_widget<-vector_default_selector_update bounded (return this_widget) fmap (\this_index this_this_selector this_vector_widget->fmap (\single_widget->DV.modify (\this_this_vector_widget->DVM.write this_this_vector_widget this_index single_widget) this_vector_widget) (selector_monad_update update this_this_selector (this_vector_widget DV.! this_index))) selector index size vector_widget
            self_selector_applicative_update maybe_value update new_widget
        Widget_trigger {next,widget_trigger,widget}->do
            new_widget<-widget_trigger_selector_update fmap id (selector_monad_update update selector) next widget_trigger widget
            self_selector_applicative_update maybe_value update new_widget
        Widget_io_trigger {next,widget_io_trigger,widget}->do
            new_widget<-widget_io_trigger_selector_update fmap id (selector_monad_update update selector) next widget_io_trigger widget
            self_selector_applicative_update maybe_value update new_widget
        Widget_mix_trigger {next,widget_mix_trigger,order,widget}->do
            new_widget<-widget_mix_trigger_selector_update fmap id (selector_monad_update update selector) next widget_mix_trigger order widget
            self_selector_applicative_update maybe_value update new_widget
    ...
    Any_selector {maybe_value,selector,strict}->case this_widget of
        Group {initial_min_index,min_index,initial_max_index,max_index,index,group_widget}->do
            new_widget<-group_all_selector_update fmap traverse (selector_monad_update update selector) initial_min_index min_index initial_max_index max_index index group_widget
            self_selector_applicative_update maybe_value update new_widget
        Vector {index,size,vector_widget}->do
            new_widget<-vector_all_selector_update fmap traverse (selector_monad_update update selector) index size vector_widget
            self_selector_applicative_update maybe_value update new_widget
        Widget_trigger {next,widget_trigger,widget}->do
            new_widget<-widget_trigger_selector_update fmap id (selector_monad_update update selector) next widget_trigger widget
            self_selector_applicative_update maybe_value update new_widget
        ...
        Widget_mix_trigger {next,widget_mix_trigger,order,widget}->do
            ...
            self_selector_applicative_update maybe_value update new_widget
        Coroutine {index,initial_min_index,min_index,initial_max_index,max_index,variable_length,user_variable_length,coroutine_state,layout,linear_coroutine,iterative}->do
            new_widget<-coroutine_all_selector_update fmap traverse (functor_update_coroutine_state (selector_monad_update update selector)) index initial_min_index min_index initial_max_index max_index variable_length user_variable_length layout linear_coroutine iterative coroutine_state
            self_selector_applicative_update maybe_value update new_widget
    ...
    Vector_selector {maybe_value,vector_selector,bounded}->case this_widget of
        Vector {index,size,vector_widget}->do
            new_vector_widget<-DIM.traverseWithKey (\this_index single_selector->if bounded then fmap Just (selector_monad_update update single_selector (vector_widget DV.! this_index)) else maybe (return Nothing) (fmap Just . selector_monad_update update single_selector) (vector_widget DV.!? this_index)) vector_selector
            self_selector_applicative_update maybe_value update (Vector {index=index,size=size,vector_widget=DV.modify (\this_vector_widget->CM.void (DIM.traverseWithKey (maybe (return ()) . DVM.write this_vector_widget) new_vector_widget)) vector_widget})
