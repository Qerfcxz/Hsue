...
main_widget_transform widget event engine=case widget of
    Double {which,first_widget}->case first_widget of
        Text {origin,matrix,width,height,y,max_y,article,charset,locked}->case event of
            Time {}->let scroll_speed=5 in let new_y=max 0 (min (max 0 (max_y-height)) (y+if DSet.member Key_w engine.key then -scroll_speed else 0+if DSet.member Key_s engine.key then scroll_speed else 0)) in if new_y/=y then (id,Double {which=which,first_widget=Text {origin=origin,matrix=matrix,width=width,height=height,y=new_y,max_y=max_y,article=article,charset=charset,locked=locked},second_widget=Store {store=Data_bool {bool=True}}}) else (id,widget)
            At {window_id=this_window_id,action}->if this_window_id==window_id
                then case action of
                    Resize {}->(id,Double {which=which,first_widget=first_widget,second_widget=Store {store=Data_bool {bool=True}}})
                    _->(id,widget)
                else (id,widget)
            _->(id,widget)
        _->(id,widget)
...
render_trigger event engine=case event of
    Time {tick}->if tick > 0&&mod tick test_time==0
        then
            let engineWithRequests = create_request Clean_atlas (create_request (Unlock {leaf_id=main_widget_id}) engine)
                (new_engine, _) = update_lookup_projection_widget path_main_widget force_redraw engineWithRequests
            in new_engine
        else
            let widget=lookup_projection_widget path_main_widget engine in case check_and_reset_data_int widget of
