...
check_and_reset_data_int widget=case widget of
    Widget_trigger {next,widget_trigger,widget=inner_widget}->case inner_widget of
        Vector {index,size,vector_widget}->case vector_widget DV.!? 1 of
            Just (Store {store=Data_bool {bool}})->if bool then Just (Widget_trigger {next=next,widget_trigger=widget_trigger,widget=Vector {index=index,size=size,vector_widget=vector_widget DV.// [(1,Store {store=Data_bool {bool=False}})]}}) else Nothing
            _->Nothing
        _->Nothing
    _->Nothing
...
force_redraw widget=case widget of
    Widget_trigger {next,widget_trigger,widget=inner_widget}->Widget_trigger {next=next,widget_trigger=widget_trigger,widget=force_redraw inner_widget}
    Vector {index,size,vector_widget}->Vector {index=index,size=size,vector_widget=vector_widget DV.// [(1,Store {store=Data_bool {bool=True}})]}
    _->widget
